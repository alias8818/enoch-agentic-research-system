...
<script>
const pages=[['overview','Overview'],['queue:active','Active'],['queue:queued','Queued'],['queue:blocked','Blocked'],['runs','Runs'],['papers','Papers'],['events','Events'],['reviews','Publication Review'],['intake','Notion Intake'],['observability','Observability']];
...
function linkProject(id){return id?`<a href="#project:${encodeURIComponent(id)}">${esc(id)}</a>`:'';} function linkRun(id){return id?`<a href="#run:${encodeURIComponent(id)}">${esc(id)}</a>`:'';} function linkPaper(id){return id?`<a href="#paper:${encodeURIComponent(id)}">${esc(id)}</a>`:'';}
function statusClass(value){const s=String(value||'').toLowerCase(); if(['blocked','dispatch_error','failed','error','critical','rejected'].some(x=>s.includes(x)))return 'bad'; if(['queued','awaiting','review','paused','warn','pending'].some(x=>s.includes(x)))return 'warn'; if(['complete','finalized','ok','ready','approved','success'].some(x=>s.includes(x)))return 'good'; return 'info';}
function cell(c,v,r){if(c==='project_id')return `${linkProject(v)}${r.project_name?`<div class="muted">${esc(r.project_name)}</div>`:''}`; if(c==='current_run_id'||c==='run_id')return linkRun(v); if(c==='paper_id')return linkPaper(v); if(c==='review')return r.paper_id?`<a href="#review:${encodeURIComponent(r.paper_id)}">Open ${esc(r.project_name||r.paper_id)}</a>`:''; if(c==='paper_title')return `<strong>${esc(r.project_name||r.paper_id||'Untitled')}</strong><div class="muted mono">${esc(r.paper_id||'')}</div>`; if(c==='notion_page_url'&&v)return `<a href="${esc(v)}">Notion</a>`; if(c==='artifact_paths_present')return Object.entries(v||{}).map(([k,ok])=>`<span class="pill ${ok?'good':'warn'}">${esc(k.replace('_path',''))} ${ok?'yes':'no'}</span>`).join(''); if(typeof v==='boolean')return `<span class="pill ${v?'good':'warn'}">${v?'yes':'no'}</span>`; if(Array.isArray(v))return `<span class="truncate">${esc(v.join('; '))}</span>`; if(v&&typeof v==='object')return Object.keys(v).length?`<span class="truncate">${esc(JSON.stringify(v))}</span>`:''; return `<span class="truncate ${statusClass(v)}">${esc(v)}</span>`;}
function tableRows(rows,cols){return `<table><thead><tr>${cols.map(c=>`<th>${esc(c.replaceAll('_',' '))}</th>`).join('')}</tr></thead><tbody>${(rows||[]).map(r=>`<tr>${cols.map(c=>`<td>${cell(c,r[c],r)}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${cols.length}">No rows</td></tr>`}</tbody></table>`;}
function pageMeta(page){return `<div class="muted">Returned ${esc(page?.returned??0)} · page size ${esc(page?.page_size??'')} · cursor ${esc(page?.cursor||'start')} ${page?.has_more?'· more available':''}</div>`;}
async function overviewPage(){renderNav('overview'); const [overview,memory,health]=await Promise.all([api('/control/api/v1/overview?active_limit=8&event_limit=8'),api('/control/api/v1/observability/memory'),api('/control/api/v1/observability/health')]); const counts=overview.counts||{}, papers=overview.paper_counts||{}; const memWarn=memory.memory_warn?'warn':'good'; $('status').className='pill '+(((counts.blocked||0)>0||memory.memory_warn)?'warn':'good'); $('status').textContent=`overview · ${counts.active||0} active · ${counts.queued||0} queued · RSS ${memory.rss_mib??'n/a'} MiB`; $('app').className=''; $('app').innerHTML=`<section class="hero"><div class="banner ${((counts.blocked||0)>0)?'warn':'good'}"><h2>Controller overview</h2><div>Status and next actions are shown first. Raw evidence is intentionally collapsed below each view.</div><div class="row"><span class="pill">source ${esc(overview.source)}</span><span class="pill">authority ${esc(overview.authority)}</span><span class="pill">generated ${esc(overview.generated_at)}</span></div></div><div class="card"><div class="label">Memory</div><div class="value ${memWarn}">${esc(memory.rss_mib??'n/a')} MiB</div><div class="muted">Peak ${esc(memory.peak_rss_mib??'n/a')} MiB · warn ${esc(memory.warn_threshold_mib??'unset')}</div></div></section><section class="grid">${card('Active lanes',counts.active||0,(counts.active||0)?'warn':'good')}${card('Queued projects',counts.queued||0,'info')}${card('Blocked / review',counts.blocked||0,(counts.blocked||0)?'warn':'good')}${card('Papers indexed',counts.papers||0,'info')}${card('Publication drafts',papers.publication_draft||0,'info')}${card('Draft review',papers.draft_review||0,'info')}${card('Route observations',health.route_observability_enabled?'enabled':'disabled',health.route_observability_enabled?'good':'warn')}${card('Next candidate',(overview.next_candidate||{}).project_name||(overview.next_candidate||{}).project_id||'none',overview.next_candidate?'info':'good')}</section><section class="grid two"><div class="card"><h2>Active lane</h2>${tableRows(overview.active_items||[],['status','project_id','current_run_id','next_action_hint','updated_at'])}</div><div class="card"><h2>Recent event summaries</h2>${tableRows(overview.recent_events||[],['event_id','event_type','entity_type','entity_id','created_at','payload_summary'])}</div></section><section class="card"><h2>Evidence/debug</h2>${debugBlock('Overview JSON',overview)}${debugBlock('Memory JSON',memory)}${debugBlock('Observability health JSON',health)}</section>`;}
async function queuePage(q){renderNav('queue:'+q); const search=new URLSearchParams(location.hash.split('?')[1]||''); const term=search.get('search')||''; const cursor=search.get('cursor')||''; const qs=new URLSearchParams({queue:q,page_size:'50',search:term,cursor}); const data=await api('/control/api/v1/queue?'+qs.toString()); $('status').className='pill info'; $('status').textContent=`${q} queue · ${data.page.returned} rows`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>${esc(q)} queue</h2>${pageMeta(data.page)}<div class="toolbar"><input id="search" value="${esc(term)}" placeholder="search project/status/action" onkeydown="if(event.key==='Enter'){location.hash='queue:${q}?search='+encodeURIComponent(this.value)}"/><button onclick="location.hash='queue:${q}?search='+encodeURIComponent($('search').value)">Search</button>${data.page.has_more?`<button onclick="location.hash='queue:${q}?search=${encodeURIComponent(term)}&cursor=${encodeURIComponent(data.page.next_cursor)}'">Next page</button>`:''}</div><div class="row">${Object.entries(data.counts||{}).map(([k,v])=>`<span class="pill">${esc(k)} ${esc(v)}</span>`).join('')}</div>${tableRows(data.rows,['status','project_id','dispatch_priority','selection_rank','last_run_state','current_run_id','next_action_hint','manual_review_required','updated_at'])}${debugBlock('Queue page JSON',data)}</section>`;}
async function runsPage(){renderNav('runs'); const data=await api('/control/api/v1/runs?page_size=50'); $('status').className='pill info'; $('status').textContent=`runs · ${data.page.returned} rows`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>Runs</h2>${pageMeta(data.page)}${tableRows(data.rows,['state','project_id','run_id','gate_state','dispatch_mode','current_activity','updated_at'])}${debugBlock('Runs page JSON',data)}</section>`;}
async function papersPage(){renderNav('papers'); const data=await api('/control/api/v1/papers?page_size=50'); $('status').className='pill info'; $('status').textContent=`papers · ${data.page.returned} rows`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>Papers</h2>${pageMeta(data.page)}<div class="row">${Object.entries(data.counts||{}).map(([k,v])=>`<span class="pill">${esc(k)} ${esc(v)}</span>`).join('')}</div>${tableRows(data.rows,['paper_status','paper_title','project_id','run_id','artifact_paths_present','updated_at'])}${debugBlock('Papers page JSON',data)}</section>`;}
async function eventsPage(){renderNav('events'); const data=await api('/control/api/v1/events?page_size=50'); $('status').className='pill info'; $('status').textContent=`events · ${data.page.returned} summaries`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>Events / audit log</h2><div class="muted">Payloads are summarized by default. Use debug APIs deliberately when raw payloads are needed.</div>${pageMeta(data.page)}${tableRows(data.rows,['event_id','event_type','entity_type','entity_id','created_at','payload_summary'])}${debugBlock('Events page JSON',data)}</section>`;}
async function observabilityPage(){renderNav('observability'); const [memory,health]=await Promise.all([api('/control/api/v1/observability/memory'),api('/control/api/v1/observability/health')]); $('status').className='pill '+(memory.memory_warn?'warn':'good'); $('status').textContent=`observability · RSS ${memory.rss_mib??'n/a'} MiB`; $('app').className=''; $('app').innerHTML=`<section class="grid three">${card('RSS',memory.rss_mib??'n/a',memory.memory_warn?'warn':'good','MiB')}${card('Peak RSS',memory.peak_rss_mib??'n/a','info','MiB')}${card('Route observations',health.route_observability_enabled?'enabled':'disabled',health.route_observability_enabled?'good':'warn')}</section><section class="card"><h2>Latest route observation</h2><div class="muted">Latest observation is read from a bounded tail of the route-observation log.</div>${health.latest_route_observation?`<details><summary>Latest route observation JSONL</summary><pre>${esc(health.latest_route_observation)}</pre></details>`:'<span class="pill warn">No observation logged</span>'}${debugBlock('Memory JSON',memory)}${debugBlock('Health JSON',health)}</section>`;}
