...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:radial-gradient(circle at 18% 0%, rgba(37,99,235,.30), transparent 30%), linear-gradient(135deg,#05070b,var(--bg)); color:var(--text); line-height:1.45; }
    ...
    .row { display:flex; flex-wrap:wrap; gap:8px; align-items:center; } .toolbar { display:flex; gap:8px; flex-wrap:wrap; align-items:center; margin:12px 0; } .activity-list { display:grid; gap:10px; } .card.tight strong { font-size:1rem; }
...
function workState(counts){const active=counts.active||0, queued=counts.queued||0, blocked=counts.blocked||0; if(active)return {label:'Work running', tone:'info', detail:`${active} active item${active===1?'':'s'} in progress`}; if(queued)return {label:'Ready to dispatch', tone:'info', detail:`${queued} queued item${queued===1?'':'s'} waiting`}; if(blocked)return {label:'Needs attention', tone:'warn', detail:`${blocked} blocked or review item${blocked===1?'':'s'}`}; return {label:'Work is idle', tone:'good', detail:'No active or queued work right now'};}
async function overviewPage(){renderNav('overview'); const [overview,memory,health]=await Promise.all([api('/control/api/v1/overview?active_limit=8&event_limit=6'),api('/control/api/v1/observability/memory'),api('/control/api/v1/observability/health')]); const counts=overview.counts||{}, papers=overview.paper_counts||{}, state=workState(counts); $('status').className='pill '+state.tone; $('status').textContent=`Overview · ${state.label} · ${counts.blocked||0} needs attention`; $('app').className=''; $('app').innerHTML=`<section class="banner ${state.tone}"><h2>${esc(state.label)}</h2><div>${esc(state.detail)}</div><div class="muted">Last refreshed ${esc(formatTime(overview.generated_at))}. Showing bounded, operator-safe summaries.</div></section><section class="grid">${card('Running now',counts.active||0,(counts.active||0)?'info':'good','Currently executing or waiting on worker callback')}${card('Ready queue',counts.queued||0,'info','Dispatchable items waiting their turn')}${card('Needs attention',counts.blocked||0,(counts.blocked||0)?'warn':'good','Blocked, manual review, or dispatch-error items')}${card('Paper lane',counts.papers||0,'info',`${papers.publication_draft||0} publication drafts · ${papers.draft_review||0} in draft review`)}</section><section class="grid two"><div class="card"><h2>Active work</h2><div class="muted">Only work currently running or awaiting callback appears here.</div>${tableRows(overview.active_items||[],['status','project_id','current_run_id','next_action_hint','updated_at'],'No active work right now')}</div><div class="card"><h2>System health</h2><div class="row"><span class="pill ${memory.memory_warn?'warn':'good'}">Controller memory ${esc(Number(memory.rss_mib||0).toFixed(0))} MiB</span><span class="pill">Peak ${esc(Number(memory.peak_rss_mib||0).toFixed(0))} MiB</span><span class="pill ${health.route_observability_enabled?'good':'warn'}">Route observations ${health.route_observability_enabled?'on':'off'}</span></div><div class="muted">Memory is shown as a secondary health signal, not the primary work status.</div></div></section><section class="card"><h2>Recent activity</h2><div class="muted">Formatted control-plane events. Large payloads stay hidden.</div>${activityCards(overview.recent_events||[])}</section>`;}
async function queuePage(q){renderNav('queue:'+q); const search=new URLSearchParams(location.hash.split('?')[1]||''); const term=search.get('search')||''; const cursor=search.get('cursor')||''; const qs=new URLSearchParams({queue:q,page_size:'50',search:term,cursor}); const data=await api('/control/api/v1/queue?'+qs.toString()); const counts=data.counts||{}; const label={active:'Active work',queued:'Ready queue',blocked:'Needs attention',paused:'Paused work',completed:'Completed work'}[q]||titleCase(q)+' queue'; $('status').className='pill '+(q==='blocked'?'warn':'info'); $('status').textContent=`${label} · showing ${data.page.returned} · system blocked ${counts.blocked||0}`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>${esc(label)}</h2><div class="muted">This table is filtered to ${esc(label.toLowerCase())}. System totals below are separate and do not mean those items are in this filtered view.</div><section class="grid"><div class="card tight"><div class="label">Showing in this view</div><div class="value">${esc(data.page.returned)}</div></div><div class="card tight"><div class="label">Running now</div><div class="value ${counts.active?'info':'good'}">${esc(counts.active||0)}</div></div><div class="card tight"><div class="label">Ready queue</div><div class="value info">${esc(counts.queued||0)}</div></div><div class="card tight"><div class="label">Needs attention</div><div class="value ${counts.blocked?'warn':'good'}">${esc(counts.blocked||0)}</div></div></section><div class="toolbar"><input id="search" value="${esc(term)}" placeholder="search project/status/action" onkeydown="if(event.key==='Enter'){location.hash='queue:${q}?search='+encodeURIComponent(this.value)}"/><button onclick="location.hash='queue:${q}?search='+encodeURIComponent($('search').value)">Search</button>${data.page.has_more?`<button onclick="location.hash='queue:${q}?search=${encodeURIComponent(term)}&cursor=${encodeURIComponent(data.page.next_cursor)}'">Next page</button>`:''}</div>${tableRows(data.rows,['status','project_id','dispatch_priority','selection_rank','last_run_state','current_run_id','next_action_hint','manual_review_required','updated_at'],`No ${label.toLowerCase()} items`)}</section>`;}
async function runsPage(){renderNav('runs'); const data=await api('/control/api/v1/runs?page_size=50'); $('status').className='pill info'; $('status').textContent=`Runs · ${data.page.returned} shown`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>Runs</h2>${pageMeta(data.page)}${tableRows(data.rows,['state','project_id','run_id','gate_state','dispatch_mode','current_activity','updated_at'])}</section>`;}
async function papersPage(){renderNav('papers'); const data=await api('/control/api/v1/papers?page_size=50'); $('status').className='pill info'; $('status').textContent=`Papers · ${data.page.returned} shown`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>Papers</h2><div class="row">${Object.entries(data.counts||{}).map(([k,v])=>`<span class="pill">${esc(titleCase(k))}: ${esc(v)}</span>`).join('')}</div>${tableRows(data.rows,['paper_status','paper_title','project_id','run_id','artifact_paths_present','updated_at'])}</section>`;}
async function eventsPage(){renderNav('events'); const search=new URLSearchParams(location.hash.split('?')[1]||''); const cursor=search.get('cursor')||''; const data=await api('/control/api/v1/events?page_size=50&cursor='+encodeURIComponent(cursor)); $('status').className='pill info'; $('status').textContent=`Activity log · ${data.page.returned} shown`; $('app').className=''; $('app').innerHTML=`<section class="card"><h2>Activity log</h2><div class="muted">Human-readable event stream. Payload fields are summarized as chips; large payloads are not printed into the table.</div>${pageMeta(data.page,'Showing latest')}${activityCards(data.rows||[])}${data.page.has_more?`<div class="toolbar"><button onclick="location.hash='events?cursor=${encodeURIComponent(data.page.next_cursor)}'">Next page</button></div>`:''}</section>`;}
