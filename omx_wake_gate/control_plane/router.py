...
function workState(counts,operators={}){const attention=operators.needs_attention ?? counts.blocked ?? 0, draft=operators.run_complete_draft_needed||0, ready=operators.ready_to_publish||0, active=counts.active||operators.running||0, queued=counts.queued||operators.idea_queued||0; if(attention)return {label:'Needs attention', tone:'warn', detail:`${attention} item${attention===1?'':'s'} need operator review`}; if(draft)return {label:'Draft needed', tone:'warn', detail:`${draft} completed run${draft===1?'':'s'} waiting on draft-next`}; if(ready)return {label:'Ready for corpus', tone:'good', detail:`${ready} finalized publication draft${ready===1?'':'s'} ready for corpus import`}; if(active)return {label:'Work running', tone:'info', detail:`${active} active item${active===1?'':'s'} in progress`}; if(queued)return {label:'Ready to dispatch', tone:'info', detail:`${queued} queued item${queued===1?'':'s'} waiting`}; return {label:'Work is idle', tone:'good', detail:'No active, queued, or paper-lane work needs attention right now'};}
async function overviewPage(){renderNav('overview'); const [overview,memory,health]=await Promise.all([api('/control/api/v1/overview?active_limit=8&event_limit=6'),api('/control/api/v1/observability/memory'),api('/control/api/v1/observability/health')]); const counts=overview.counts||{}, papers=overview.paper_counts||{}, ops=overview.operator_counts||{}, state=workState(counts,ops); $('status').className='pill '+state.tone; $('status').textContent=`Overview · ${state.label} · ${ops.needs_attention ?? counts.blocked ?? 0} needs attention`; $('app').className=''; $('app').innerHTML=`<section class="banner ${state.tone}"><h2>${esc(state.label)}</h2><div>${esc(state.detail)}</div><div class="muted">Last refreshed ${esc(formatTime(overview.generated_at))}. Showing bounded, operator-safe summaries.</div></section><section class="grid">${card('Running now',counts.active||0,(counts.active||0)?'info':'good','Currently executing or waiting on worker callback')}${card('Ready queue',counts.queued||0,'info','Dispatchable items waiting their turn')}${card('Needs attention',ops.needs_attention ?? counts.blocked ?? 0,(ops.needs_attention ?? counts.blocked ?? 0)?'warn':'good','Blocked, worker question, manual-review, or dispatch-error queue items')}${card('Ready for corpus',(overview.operator_counts||{}).ready_to_publish||0,'good','Finalized publication drafts ready for corpus import')}${card('Paper automation',(overview.operator_counts||{}).finalization_needed||0,'info','Publication packages waiting for the automated finalizer')}${card('Draft needed',(overview.operator_counts||{}).run_complete_draft_needed||0,'warn','Completed worker runs waiting on draft-next')}${card('Paper lane',counts.papers||0,'info',`${papers.publication_draft||0} publication drafts · ${papers.draft_review||0} in draft review`)}</section><section class="grid two"><div class="card"><h2>Active work</h2><div class="muted">Only work currently running or awaiting callback appears here.</div>${tableRows(overview.active_items||[],['operator_stage_label','project_id','current_run_id','operator_next_step','updated_at'],'No active work right now')}</div><div class="card"><h2>System health</h2><div class="row"><span class="pill ${memory.memory_warn?'warn':'good'}">Controller memory ${esc(Number(memory.rss_mib||0).toFixed(0))} MiB</span><span class="pill">Peak ${esc(Number(memory.peak_rss_mib||0).toFixed(0))} MiB</span><span class="pill ${health.route_observability_enabled?'good':'warn'}">Route observations ${health.route_observability_enabled?'on':'off'}</span></div><div class="muted">Memory is shown as a secondary health signal, not the primary work status.</div></div></section><section class="card"><h2>Recent activity</h2><div class="muted">Formatted control-plane events. Large payloads stay hidden.</div>${activityCards(overview.recent_events||[])}</section>`;}
