...
- [ ] Read-model hardening (ongoing) — extend DTO boundary tests ([#97](../dashboard/src/api/readModelSchemas.ts)) when adding overview/lane/intake fields; fix semantics in backend first per cursor instructions.
  ...
  - [x] **Intake/ideas boundary (2026-05-23)** — `intakeIdeasResponseSchema` + `parseIntakeIdeasResponse` in [`readModelSchemas.ts`](../dashboard/src/api/readModelSchemas.ts); [`IntakePage`](../dashboard/src/components/ResourcePages.tsx) parses at query boundary; Vitest in [`readModelSchemas.test.ts`](../dashboard/src/api/readModelSchemas.test.ts); aligns with `test_dashboard_intake_ideas_projection_includes_operator_fields`.
  - [x] **B7 pause/maintenance (2026-05-23)** — `test_overview_flags_reflect_dashboard_v2_pause_maintenance_mode` in [`test_control_plane_router.py`](../tests/test_control_plane_router.py); cutover audit B7 marked Pass.
