...
flowchart TD
  ...
  J --> K[GB10 worker gate / Enoch Control Plane run]
...
- **Single active dispatch lane.** `ControlPlaneStore.next_dispatch_candidate()` refuses a candidate when the queue is paused or `active_items()` is non-empty, and sorts queued items by `dispatch_priority`, `selection_rank`, then `updated_at` (`enoch_control_plane/control_plane/store.py:1198-1207`). `/control/dispatch-next` repeats the active/candidate checks before live dispatch (`enoch_control_plane/control_plane/router.py:1506-1517`).
- **Dispatch start is durable.** Accepted live dispatch updates the queue row to `awaiting_wake`, sets `current_run_id`, clears stale error/result fields, creates/updates the run row, and appends a `controller.live_dispatch` event (`enoch_control_plane/control_plane/store.py:1427-1469`).
- **Wake callbacks are state-shaping and auditable.** `question_pending` goes to `needs_review`, gate failures go to `blocked`, ready callbacks go to `completed` with `draft_paper_or_select_next_project`, and unknown direct-store callback events go to manual review (`enoch_control_plane/control_plane/store.py:1471-1530`). The audit fix adds the missing `session_started -> running/await_callback` branch.
- **Paper draft eligibility is deterministic.** Candidates must be `completed`, paper-ready by final positive or wake-ready next-action, not manual-review, not already drafted by project or run, and not in excluded human-label benchmark categories (`enoch_control_plane/enoch_core/logic.py:155-204`).
- **Paper-positive gate is conservative.** For LangGraph wake-ready rows, `/control/papers/draft-next` requires local/synced evidence plus `paper_draft_decision_gate()`; the gate blocks negative, inconclusive, needs-review, caveat-only, and negated-positive decisions before accepting exact positive/proceed/viable tokens (`enoch_control_plane/control_plane/router.py:1602-1651`, `enoch_control_plane/enoch_core/logic.py:86-116`).
- **Publication automation has explicit state.** Publication automation rows track rewrite/finalization state separately from operator attention; finalization is reserved for dedicated endpoints and does not imply human approval or external submission.
- **Finalization is package-only.** Preparing finalization requires `approved_for_finalization`, readable artifacts, writes a local manifest, and records `no_submission_side_effects=true` (`enoch_control_plane/control_plane/store.py:1004-1056`).

Risk: `ACTIVE_QUEUE_STATUSES` in `enoch_control_plane/enoch_core/logic.py:9` is narrower than control-plane `ACTIVE_STATUSES` in `enoch_control_plane/control_plane/store.py:22`, which also includes `wake_received` and `reconciling`. This is low-to-medium risk because projection warnings can under-report transitional active states even if dispatch itself uses the control-plane store.
...
- `enoch_control_plane/control_plane/store.py:1473-1535` maps worker callback events to durable queue state:
...
- The callback endpoint now converts idempotency conflicts to HTTP `409` in `enoch_control_plane/control_plane/router.py:884-904`.
...
- Branch successor audit support exists in `enoch_control_plane/enoch_core/logic.py:139-152`: `branch_queued` rows require successor project ID evidence and a Notion successor URL.
...
## Legacy Notion intake, projection, and idempotency audit

- Notion page normalization and current API data-source querying are in `enoch_control_plane/control_plane/notion_sync.py:65-120`.
- Execution update projection shapes safe Notion properties in `enoch_control_plane/control_plane/notion_sync.py:149-225`; it requires explicit `page_id` and filters PATCH payloads to existing Notion page properties.
- Control-plane intake normalizes titles/status/page IDs/priorities in `enoch_control_plane/control_plane/store.py:87-128` and creates deterministic project IDs from page IDs in `enoch_control_plane/control_plane/store.py:1224-1258`.
- Intake commit records an idempotent event before table mutation in `enoch_control_plane/control_plane/store.py:1261-1273`.
- Execution update rows map queue/paper state back to Notion overlay fields in `enoch_control_plane/control_plane/store.py:1319-1355`.
...
## Paper drafting, automation, rewrite, duplicate, and positive-gate audit
...
### Automation, rewrite, and finalization

- Review status and checklist definitions are explicit in `enoch_control_plane/control_plane/store.py:131-150`.
- Review mutations are idempotent/event-logged in `enoch_control_plane/control_plane/store.py:880-982`.
- Finalization packaging in `enoch_control_plane/control_plane/store.py:1004-1055` requires `approved_for_finalization`, readable artifacts for non-dry-run, includes checklist/artifact manifest, and records `no_submission_side_effects: true`.
- Publication rewrite in `enoch_control_plane/control_plane/router.py:1156-1223` uses a VM-local artifact root when needed, syncs evidence, preserves an AI-generated/no-human-credit publication policy, writes updated artifacts, and sets `publication_draft`.
- Batch rewrite in `enoch_control_plane/control_plane/router.py:1225-1259` skips finalized/rejected rows and can skip already rewritten rows by checking `paper_review.draft_rewritten` events.

Superseded risk: older checklist/final-human-approval language reflected a manual review-era workflow. Current operator semantics should present this lane as automated publication finalization, not human paper approval or peer review.

## Findings ranked by risk

| Risk | Finding | Evidence | Recommendation |
| --- | --- | --- | --- |
| Queue/run state vocabulary | Queue, run, paper, and review states are centralized as enums. | `enoch_control_plane/control_plane/models.py:11`, `enoch_control_plane/control_plane/models.py:26`, `enoch_control_plane/control_plane/models.py:40`, `enoch_control_plane/control_plane/models.py:50` | Good: public state names are explicit. Hardening: add a transition table for queue statuses equivalent to the paper-review transition table so callback/import paths cannot introduce impossible queue edges. |
| Single active lane | Active rows are `dispatching`, `awaiting_wake`, and `running`; dispatch dry-run/live paths refuse a second active lane. | `enoch_control_plane/enoch_core/logic.py:9`, `enoch_control_plane/enoch_core/logic.py:127`, `enoch_control_plane/control_plane/store.py:1214`, `enoch_control_plane/control_plane/router.py:1507` | Good safety invariant. Hardening: include `wake_received`/`reconciling` consistently across all active-lane helpers or document why the core helper intentionally excludes them. |
...
- `enoch_control_plane/control_plane/store.py` — Notion intake replay no-op after existing idempotency event; worker callback replay/conflict check before queue/run mutation; `session_started` stays active as `running` / `await_callback`.
- `enoch_control_plane/control_plane/router.py` — worker callback idempotency conflicts return HTTP `409`.
