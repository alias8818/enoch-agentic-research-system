...
| 6 | Paper -> Publication | `papers.paper_status`, `publication_automation_items` | Publication automation / finalizer | Draft exists and automation produces a finalization package | Human approval/review vocabulary as the normal path; editorial taste review holding generated papers | `automate_publication` |
| 7 | Publication -> Corpus | Required evidence paths plus `publication_automation_items.finalized` and `corpus_imports` ledger | Corpus import script and public release validator | Publication draft has required evidence paths and finalized package; public corpus index includes sanitized artifact by source fingerprint; control-plane ledger records the import | Counting all finalized rows as actionable publish/import work; waiting on human rewrite after gates pass | `ready_to_publish` only while missing corpus import, then `published`/public corpus evidence |
...
11. Paper-positive artifacts auto-push as written once deterministic gates pass. Manual editorial approval, taste review, or agent opinion must not be inserted as a normal transition between writer output, finalization, corpus import, and public corpus push.
