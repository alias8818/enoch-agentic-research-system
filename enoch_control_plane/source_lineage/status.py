...
SOURCE_PROBLEM_TOKENS = (
    ...
    "source_url_missing_source",
    "source_id_missing_source",
)
LINEAGE_PROBLEM_TOKENS = (
    "missing_lineage",
    "missing_parent_project_lineage",
    "missing_parent_run_lineage",
)
...
def classify_source_lineage_report(
    ...
    if (
        ...
        or not isinstance(problem_counts_raw, dict)
        or not isinstance(problems_raw, list)
    ):
        return {
            "ok": False,
            "status": "blocked",
    ...
    problem_counts = {
        str(key): _safe_int(value) for key, value in problem_counts_raw.items()
    }
    missing_sources, missing_lineage = _problem_buckets(problem_counts)
    problem_total = _safe_int(counts_raw.get("problems")) or sum(
        problem_counts.values()
    )
    status = (
        str(report.get("status") or ("blocked" if problem_total else "clean"))
        .strip()
        .lower()
    )
    ...
    return {
        ...
        "problem_details": [item for item in problems_raw if isinstance(item, dict)][
            :25
        ],
    }


def load_latest_source_lineage_status(
    paths: tuple[str, ...] | list[str] = (DEFAULT_REPORT_PATH,),
) -> dict[str, Any]:
