...
<div class="app-shell">
  <aside class="sidebar">
    ...
    <div class="sidebar-footer">
      <div class="footer-line"><span class="build-tag">v1 read models</span></div>
      <div class="footer-line">Bounded Supabase read models · raw states stay in drill-down views</div>
    </div>
  </aside>
  <main class="dashboard-main">
    <div class="topbar">
      <div class="search-box"><input id="globalSearch" placeholder="Search projects, runs, papers, or events…" onkeydown="if(event.key==='Enter')globalSearch()" /><span class="kbd-hint" aria-hidden="true">/</span></div>
      <div class="topbar-actions">
        <div id="systemState" class="system-state info" title="System status">Initialising…</div>
        <button id="themeToggle" type="button" class="theme-toggle" aria-pressed="false" aria-label="Toggle color theme" title="Toggle theme" onclick="cycleTheme()">A</button>
...
function sparklineTrend(value,cls='info'){const n=Math.max(0,Math.min(9999,Number(value)||0)); const tier=n>=20?'high':n>=5?'mid':n>0?'low':'zero'; return `<div class="sparkline sparkline--snapshot tier-${tier} ${esc(cls||'info')}" role="img" aria-label="current snapshot indicator (not a trend)" title="Snapshot indicator. Not historical trend data."><span class="sparkline__bar"><span class="sparkline__fill"></span></span><span class="sparkline__label">snapshot</span></div>`;}
...
function formatTime(value){if(!value)return ''; const d=new Date(value); if(Number.isNaN(d.getTime()))return esc(value); return d.toLocaleString([], {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'});}function formatRelativeAge(value){if(!value)return 'unknown'; const d=new Date(value); if(Number.isNaN(d.getTime()))return String(value); const sec=Math.max(0,Math.round((Date.now()-d.getTime())/1000)); if(sec<2)return 'just now'; if(sec<60)return sec+'s ago'; const min=Math.round(sec/60); if(min<60)return min+'m ago'; const hr=Math.round(min/60); if(hr<48)return hr+'h ago'; return Math.round(hr/24)+'d ago';}
...
function lanePrimaryReason(l){if(l.dispatch_available)return 'Ready to dispatch'; if(l.active_item)return `Running ${l.active_item.project_name||l.active_item.project_id||'current project'}`; return l.dispatch_blocker||'No queued candidate for lane';}
function laneCommandCenter(status={}){const lanes=(status.worker_lanes||[]).filter(l=>['CPU lane','GB10 lane'].includes(workerLaneLabel(l))), loadedAt=status.generated_at||new Date().toISOString(); const visible=lanes.length?lanes:(status.worker_lanes||[]); const cards=visible.map(l=>{const label=workerLaneLabel(l), active=l.active_item||null, next=l.next_candidate||null, feed=l.feed_pressure||{}, feedAction=String(feed.next_autopilot_action||'observe'), queued=Number(l.queued_count||0), canDispatch=!!l.dispatch_available, canFeed=['generate_candidate','promote_candidate'].includes(feedAction), tone=l.status==='active'?'info':canDispatch?'good':canFeed?'warn':'muted', primary=lanePrimaryReason(l), disabled=canDispatch?'':primary; return `<article class="lane-card ${tone}"><div class="lane-card__top"><div><div class="label">${esc(label)}</div><h3>${esc(l.status||'unknown')}</h3></div><span class="pill ${tone}">${esc(primary)}</span></div><div class="lane-card__body"><div><strong>${active?'Current':next?'Next':'Queue'}</strong><div class="muted">${active?esc(active.project_name||active.project_id||'active project'):next?esc(next.project_name||next.project_id||'queued project'):esc(l.dispatch_blocker||'No queued candidate for lane')}</div></div><div class="lane-stat"><span>${esc(queued)}</span><small>queued</small></div></div><div class="lane-card__feed"><strong>Autopilot next:</strong><div class="muted">${esc(titleCase(feedAction.replaceAll('_',' ')))} · ${esc(feed.operator_summary||primary)}</div></div><div class="lane-card__actions"><button class="button-secondary" onclick="runResearchCycle(false)" ${canFeed?'':'disabled'} title="${canFeed?'Generate or promote work for this lane.':esc(feed.operator_summary||'Lane feed is already satisfied.')}">Feed idle lane</button><button class="button-primary" onclick="startNextDispatch()" ${canDispatch?'':'disabled'} title="${canDispatch?'Dry-run first, then dispatch an open lane.':esc(disabled)}">Dispatch this lane</button></div></article>`;}).join('')||'<div class="empty-state"><div><strong>No worker lanes configured</strong><div>Configure CPU and GB10 worker targets to expose lane capacity.</div></div></div>'; return `<section class="command-center" id="workerLanes"><div class="section-head"><div><h2>Worker lanes</h2><div class="section-copy">CPU and GB10 are the center of the operator view. Each lane shows whether work is running, queued, feedable, or dispatchable; includes the next autopilot feed action for each lane.</div><div class="muted">Last loaded ${esc(formatRelativeAge(loadedAt))}</div></div></div><div class="lane-grid">${cards}</div></section>`;}
function workerLaneCards(status={}){return laneCommandCenter(status);}
...
function dispatchOneControls(queued){return `<div class="toolbar"><input id="dispatchOneProjectId" placeholder="queued project id"/><button class="button-primary" title="${queued?'Dry-run, then dispatch exactly one selected queued project.':'Disabled because the queue is empty.'}" onclick="dispatchSelectedProject()" ${queued?'':'disabled'}>Dispatch selected queued project</button></div><div class="muted">Single-project dispatch works even while the broad queue is paused. It dry-runs the selected project first and never starts a batch drain.</div>`;}
function operatorCommandPanel(counts={}){const queued=Number(counts.queued||0), active=Number(counts.active||0); return `<section class="action-panel" id="commandPanel"><div class="section-head"><div><h2>Commands</h2><div class="section-copy">Authenticated operator controls. Start next does a dry-run first. Destructive, neutral, and live-dispatch actions are visually separated.</div></div></div><div class="action-grid"><div class="action-group"><h3>Queue control</h3><div class="toolbar"><button class="button-danger" onclick="pauseQueue()">Pause queue</button><button onclick="resumeQueue()">Resume queue</button></div></div><div class="action-group"><h3>Dispatch</h3><div class="toolbar"><button class="button-primary" onclick="dryRunDispatchNext()">Check next dispatch</button><button class="button-primary" title="Loading lane capacity before enabling live dispatch." onclick="startNextDispatch()" disabled>Start next queued item</button></div></div><div class="action-group"><h3>Controlled one-off dispatch</h3>${dispatchOneControls(queued)}</div></div><div id="commandStatus" class="muted">Queued ${esc(queued)} · active ${esc(active)}. Loading lane capacity…</div></section>`;}
...
async function refreshCommandPanel(){const el=$('commandPanel'); if(!el)return; try{const existingProjectId=$('dispatchOneProjectId')?.value||''; const state=await api('/control/state'); const flags=state.flags||{}, counts=state.counts||{}, next=state.next_candidate||null, lanes=state.worker_lanes||[]; const queued=Number(counts.queued||0), active=Number(counts.active||0), paused=Boolean(flags.queue_paused), maintenance=Boolean(flags.maintenance_mode), laneOpen=lanes.some(l=>l.dispatch_available), dispatchDisabledReason='No queued item matches an idle worker lane. Idle lanes without queued candidates cannot start work.'; el.innerHTML=`<div class="section-head"><div><h2>Commands</h2><div class="section-copy">Authenticated operator controls. Start next does a dry-run first. Destructive, neutral, and live-dispatch actions are visually separated.</div></div></div><div class="action-grid"><div class="action-group"><h3>Queue control</h3><div class="toolbar"><button class="button-danger" onclick="pauseQueue()" ${paused?'disabled':''}>Pause queue</button><button onclick="resumeQueue()" ${!paused&&!maintenance?'disabled':''}>Resume queue</button></div></div><div class="action-group"><h3>Dispatch</h3><div class="toolbar"><button class="button-primary" onclick="dryRunDispatchNext()">Check next dispatch</button><button class="button-primary" title="${laneOpen&&!paused&&!maintenance?'Dry-run first, then start the next queued item on an open worker lane.':dispatchDisabledReason}" onclick="startNextDispatch()" ${laneOpen&&!paused&&!maintenance?'':'disabled'}>Start next queued item</button></div></div><div class="action-group"><h3>Controlled one-off dispatch</h3>${dispatchOneControls(queued)}</div></div><div id="commandStatus" class="banner ${paused||maintenance?'warn':'good'}"><strong>${paused||maintenance?'Queue paused':'Queue open'}</strong><div>Queued ${esc(queued)} · active ${esc(active)} · maintenance ${maintenance?'on':'off'}</div><div class="muted">${next?`Next: ${esc(next.project_name||next.project_id)} · ${esc((lanes.find(l=>l.next_candidate&&l.next_candidate.project_id===next.project_id)||{}).machine_target||'default')}`:dispatchDisabledReason}</div>${laneCommandSummary(lanes)}</div>`; const input=$('dispatchOneProjectId'); if(input&&existingProjectId)input.value=existingProjectId;}catch(e){if(e.name==='AbortError')return; const status=$('commandStatus'); if(status){status.className='banner critical'; status.textContent='Command state unavailable: '+e.message;}}}
...
function setSystemState(tone,label){const el=$('systemState'); if(!el)return; el.className='system-state '+(tone||'info'); el.textContent=String(label||'Status unknown');}
function applyTheme(mode){const root=document.documentElement; const safe=mode==='light'||mode==='dark'?mode:'auto'; if(safe==='auto')root.removeAttribute('data-theme'); else root.setAttribute('data-theme',safe); const btn=$('themeToggle'); if(btn){const glyph=safe==='light'?'☀':safe==='dark'?'☾':'A'; btn.textContent=glyph; btn.setAttribute('aria-pressed', safe==='auto'?'false':'true'); btn.title='Theme: '+safe+' (click to cycle)';}}
function cycleTheme(){const cur=localStorage.getItem('enochDashboardTheme')||'auto'; const next=cur==='auto'?'light':cur==='light'?'dark':'auto'; localStorage.setItem('enochDashboardTheme',next); applyTheme(next);}
function initTheme(){applyTheme(localStorage.getItem('enochDashboardTheme')||'auto');}
function heroStatusCard(state,counts={},papers={},generatedAt='',diagnosis={}){return leaveRunningHero(state,counts,papers,generatedAt,diagnosis);}
function leaveRunningHero(state,counts={},papers={},generatedAt='',diagnosis={}){const status=diagnosis.status||state?.tone||'unknown', tone=status==='ready'?'good':status==='actionable'?'info':'warn', answer=status==='ready'?'Yes':status==='actionable'?'Yes, but there is work you can start':'No'; const reason=diagnosis.primary_reason||state?.detail||'No deterministic status returned.'; return `<section class="hero-status command-hero ${tone}"><div class="hero-status__inner"><div class="hero-status__lead"><div class="hero-status__label">Can I leave this running?</div><h2 class="hero-status__title">${esc(answer)}</h2><div class="hero-status__detail">${esc(reason)}</div><div class="hero-status__meta"><span class="pill ${tone}">${esc(status.toUpperCase())}</span><span class="pill">Last refreshed ${esc(formatRelativeAge(generatedAt))}</span><span class="pill">Server-diagnosed</span></div></div><div class="kpi-strip compact"><div class="kpi"><div class="kpi-label">Active</div><div class="kpi-value ${counts.active?'info':'good'}">${esc(counts.active||0)}</div></div><div class="kpi"><div class="kpi-label">Queued</div><div class="kpi-value ${counts.queued?'info':'good'}">${esc(counts.queued||0)}</div></div><div class="kpi"><div class="kpi-label">Drafts</div><div class="kpi-value ${(Number(papers.publication_draft||0)+Number(papers.draft_review||0))?'warn':'good'}">${esc(Number(papers.publication_draft||0)+Number(papers.draft_review||0))}</div></div></div></div></section>`;}
function actionToneClass(t){const v=String(t||'').toLowerCase(); return ['warn','bad','critical','info','good'].includes(v)?v:'info';}
function primaryActionPanel(actions=[],state={}){const a=(actions||[])[0]; if(!a){return `<section class="primary-action action-row good"><div><div class="label">Primary action</div><h2>Nothing to click right now</h2><div class="muted">${esc(state?.detail||'The action model did not rank an operator action.')}</div></div></section>`;} const tone=actionToneClass(a.tone), hash=esc(a.action_hash||'#overview'); return `<section class="primary-action action-row ${tone}"><div><div class="label">Primary action</div><h2>${esc(a.title||'Action')}</h2><div class="muted">${esc(a.summary||'')}</div></div><a class="button-primary" href="${hash}">${esc(a.action_label||'Open')}</a></section>`;}
function topActionsCard(actions=[],state={}){/* Legacy heading preserved for regression: What needs me right now? */ return primaryActionPanel(actions,state);}
function safetyBar(flags={}){const paused=!!flags.queue_paused, maintenance=!!flags.maintenance_mode; return `<section class="safety-bar"><div><strong>Queue safety</strong><span class="muted"> paused=${paused?'true':'false'} · maintenance=${maintenance?'on':'off'}</span></div><div class="toolbar"><button class="button-danger" onclick="pauseQueue()" ${paused?'disabled':''}>Pause queue</button><button onclick="resumeQueue()" ${!paused&&!maintenance?'disabled':''}>Resume queue</button></div></section>`;}
function paperPipelineMiniStrip(pipeline={}){const write=Number(pipeline.write_needed||0), finalize=Number(pipeline.finalize_needed||0), publish=Number(pipeline.publish_ready||0); const step=(label,count,hash)=>`<a class="paper-mini__step ${count?'warn':'good'}" href="${hash}"><span>${esc(label)}</span><strong>${esc(count)}</strong></a>`; return `<section class="paper-mini stepper-modern"><div class="section-head"><div><h2>Paper pipeline</h2><div class="section-copy">Tiny action strip only. Full paper counts are below the fold.</div></div></div><div class="paper-mini__steps">${step('Write',write,'#papers?status=publication_draft')}${step('Finalize',finalize,'#automation')}${step('Publish',publish,'#corpus')}</div></section>`;}
function pipelineStepperCard(pipeline={}){return paperPipelineMiniStrip(pipeline);}
function movementDiagnosisPanel(diagnosis={}){const blockers=diagnosis.blockers||[]; const rows=blockers.length?blockers.map(b=>`<div class="diagnosis-row ${esc(b.tone||'warn')}"><div><strong>${esc(b.title||b.kind||'Blocker')}</strong><div class="muted">${esc(b.summary||'')}</div></div>${b.action_hash?`<a href="${esc(b.action_hash)}">${esc(b.action_label||'Open')}</a>`:''}</div>`).join(''):'<div class="diagnosis-row good"><div><strong>No blocker found</strong><div class="muted">No deterministic blocker is preventing movement.</div></div></div>'; return `<section class="movement-diagnosis"><div class="section-head"><div><h2>Why no work is moving?</h2><div class="section-copy">Deterministic blocker list. This is the panel that should answer the repeated operational question.</div></div><span class="pill ${diagnosis.status==='ready'?'good':diagnosis.status==='actionable'?'info':'warn'}">${esc(diagnosis.status||'unknown')}</span></div><div class="diagnosis-stack">${rows}</div></section>`;}
function secondaryDetails(overview,counts,ops,pipeline,investigation){return `<details id="overviewSecondary" class="section-panel overview-secondary"><summary>Show secondary details — Active work, system health, and recent activity</summary><div class="muted">Historical counts, debug cards, active work, health, and recent activity stay below the hard fold.</div><section class="grid two"><div class="card"><h2>Active work</h2><div class="muted">Only work currently running or awaiting callback appears here.</div>${tableRows(overview.active_items||[],['operator_stage_label','project_id','current_run_id','operator_next_step','updated_at'],'No active work right now')}</div><div class="card" id="overviewHealth"><h2>System health</h2><div class="skeleton"></div><div class="muted">Secondary health checks load after the primary cards render.</div></div></section><section class="card"><h2>Recent activity</h2><div class="muted">Formatted control-plane events. Large payloads stay hidden.</div>${activityCards(overview.recent_events||[])}</section>${operatorQuestionCards(counts,ops,pipeline,investigation)}${investigationPipelineCards(investigation)}${paperPipelineCards(pipeline,overview.paper_counts||{})}</details><details id="operatorDetailBreakdown" class="section-panel"><summary>Operator detail breakdown</summary><div class="muted">Compatibility drill-down retained for existing operator semantics.</div>${debugBlock('Overview JSON',overview)}</details>`;}

async function overviewPage(){const routeKey=(location.hash||'#overview').slice(1).split('?')[0]||'overview'; renderNav('overview'); const app=$('app'), hasOverview=app?.dataset?.page==='overview'; $('status').className='pill warn'; $('status').textContent=hasOverview?'Refreshing overview…':'Loading overview…'; if(!hasOverview){app.className=''; app.innerHTML='<section class="hero-status"><div class="hero-status__inner"><div class="hero-status__lead"><div class="hero-status__label">Can I leave this running?</div><h2 class="hero-status__title">Checking…</h2><div class="hero-status__detail">Fetching bounded operator state.</div></div><div class="kpi-strip"><div class="skeleton" style="min-height:64px"></div><div class="skeleton" style="min-height:64px"></div><div class="skeleton" style="min-height:64px"></div></div></div></section>';} const overview=await api('/control/api/v1/overview?active_limit=8&event_limit=6'); if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const counts=overview.counts||{}, papers=overview.paper_counts||{}, ops=overview.operator_counts||{}, pipeline=overview.paper_pipeline||{}, investigation=overview.investigation_pipeline||{}, topActions=Array.isArray(overview.top_actions)?overview.top_actions:[], diagnosis=overview.movement_diagnosis||{}, state=workState(counts,ops,pipeline,investigation); $('status').className='pill '+(diagnosis.status==='ready'?'good':diagnosis.status==='actionable'?'info':state.tone); $('status').textContent=`Overview · ${diagnosis.status||state.label}`; setSystemState(diagnosis.status==='ready'?'good':diagnosis.status==='actionable'?'info':state.tone,diagnosis.status||state.label); $('app').dataset.page='overview'; $('app').className=''; $('app').innerHTML=`${leaveRunningHero(state,counts,papers,overview.generated_at,diagnosis)}${safetyBar(overview.flags||{})}${primaryActionPanel(topActions,state)}<section class="section-panel" id="workerLanes"><div class="section-head"><div><h2>Worker lanes</h2><div class="section-copy">Loading CPU lane and GB10 lane capacity…</div></div></div><section class="lane-grid"><div class="skeleton"></div><div class="skeleton"></div></section></section>${paperPipelineMiniStrip(pipeline)}${movementDiagnosisPanel(diagnosis)}${secondaryDetails(overview,counts,ops,pipeline,investigation)}`; api('/control/api/status').then(status=>{if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('workerLanes'); if(el)el.outerHTML=laneCommandCenter(status);}).catch(e=>{if(e.name==='AbortError')return; if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('workerLanes'); if(el)el.innerHTML=`<h2>Worker lanes</h2><span class="pill warn">Lane capacity unavailable</span><div class="muted">${esc(e.message)}</div>`;}); api('/control/api/v1/automation-readiness').then(readiness=>{if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const diag=$('overviewSecondary'); if(diag)diag.insertAdjacentHTML('beforeend', automationReadinessCard(readiness));}).catch(e=>{if(e.name==='AbortError')return;}); Promise.all([api('/control/api/v1/observability/memory'),api('/control/api/v1/observability/health')]).then(([memory,health])=>{if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('overviewHealth'); if(!el)return; el.innerHTML=`<h2>System health</h2><div class="row"><span class="pill ${memory.memory_warn?'warn':'good'}">Controller memory ${esc(Number(memory.rss_mib||0).toFixed(0))} MiB</span><span class="pill">Peak ${esc(Number(memory.peak_rss_mib||0).toFixed(0))} MiB</span><span class="pill ${health.route_observability_enabled?'good':'warn'}">Route observations ${health.route_observability_enabled?'on':'off'}</span></div><div class="muted">Memory is shown as a secondary health signal, not the primary work status.</div>`;}).catch(e=>{if(e.name==='AbortError')return; if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('overviewHealth'); if(el)el.innerHTML=`<h2>System health</h2><span class="pill warn">Health unavailable</span><div class="muted">${esc(e.message)}</div>`;});}
...
window.addEventListener('hashchange',route); initTheme(); route(); setInterval(autoRefreshCurrentPage,15000);
</script>
</body>
</html>
"""
...
def create_control_plane_router(config: GateConfig, require_bearer: RequireBearer) -> APIRouter:
    ...
    def _worker_lane_capacity(*, active: list[dict[str, Any]] | None = None, rows: list[dict[str, Any]] | None = None, global_blockers: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        for lane in sorted(lanes_by_key.values(), key=lambda item: (str(item.get("worker_role") or ""), str(item.get("machine_target") or ""), str(item.get("lane_key") or ""))):
            lane_key = str(lane["lane_key"])
            lane_active = sorted(active_by_lane.get(lane_key, []), key=lambda row: str(row.get("updated_at") or ""), reverse=True)
            ...
            output.append({
                **lane,
                ...
                "active_item": _worker_lane_summary_row(lane_active[0] if lane_active else None),
    ...
    def dashboard_v1_overview(
        ...
        worker_lanes = _worker_lane_capacity(active=active_for_lanes, rows=queued_for_lanes)
        try:
            overview_min_admission_score = float(os.environ.get("ENOCH_RESEARCH_ADMIT_THRESHOLD") or 72.0)
        except ValueError:
        ...
        open_candidate = _open_worker_dispatch_candidate(active=active_for_lanes, queued=queued_for_lanes)
