...
function operatorQuestionCards(counts={},operators={},pipeline={},investigation={}){const attention=Number(operators.needs_attention ?? counts.blocked ?? 0), running=Number(counts.active||operators.running||0), write=Number(pipeline.write_needed||0), useful=Number(investigation.useful_signals||operators.useful_signal||0), scale=Number(investigation.compute_scale_blocked||operators.compute_scale_blocked||0), follow=Number(investigation.followup_needed||operators.followup_investigation||0), publish=Number(pipeline.publish_ready||operators.ready_to_publish||0), done=Number(operators.complete_no_paper||0); return `<section class="card"><h2>What do I need to know?</h2><div class="muted">Primary dashboard cards answer operator questions first. Raw states stay in drill-down/debug views.</div><section class="grid">${card('What needs me?',attention,attention?'warn':'good',attention?'Open Needs attention and resolve blockers/questions.':'Nothing needs operator action.')}${card('What is running?',running,running?'info':'good',running?'Work is executing or waiting on callback.':'No active worker lane.')}${card('What is useful?',useful,useful?'info':'good',useful?'Bounded local signals worth preserving or deepening.':'No non-paper useful signals waiting.')}${card('Compute-scale blocked?',scale,scale?'info':'good',scale?'Promising signals parked because the next test exceeds local compute/time.':'No scale-blocked promising signals.')}${card('What can be written?',write,write?'warn':'good',write?'Paper-positive or paper-scout bounded runs need drafts.':'No actionable paper-ready runs need drafts.')}${card('Needs another investigation?',follow,follow?'info':'good',follow?'No-paper rows with specific adjacent follow-up evidence.':'No bounded follow-up candidates.')}${card('What can be published?',publish,publish?'warn':'good',publish?'Finalized drafts are missing corpus import.':'No finalized drafts are missing corpus import.')}${card('What is done / no paper?',done,'muted','Completed non-positive or non-writable rows; no paper action.')}</section></section>`;}
function investigationPipelineCards(investigation={}){const follow=Number(investigation.followup_needed||0), useful=Number(investigation.useful_signals||0), scale=Number(investigation.compute_scale_blocked||0), next=investigation.next_followup_candidate||null, nextUseful=investigation.next_useful_signal||null, nextScale=investigation.next_compute_scale_blocked||null; const nextText=next?`Next: ${esc(next.followup_title||next.project_name||next.project_id||'untitled')}`:'No bounded follow-up candidate'; const usefulText=nextUseful?`Next: ${esc(nextUseful.project_name||nextUseful.project_id||'untitled')}`:'No useful signal waiting'; const scaleText=nextScale?`Next: ${esc(nextScale.project_name||nextScale.project_id||'untitled')}`:'No scale-blocked signal'; const button=follow?'<div class="toolbar"><button onclick="launchNextFollowup()">Launch follow-up</button></div>':''; return `<section class="card"><h2>Investigation follow-ups</h2><div class="muted">Useful signals are preserved separately from papers. Follow-ups are bounded investigation work and do not become papers unless the new run independently becomes paper-positive.</div><section class="grid">${card('Useful signals',useful,useful?'info':'good',usefulText)}${card('Needs another investigation',follow,follow?'info':'good',nextText)}${card('Scale blocked',scale,scale?'info':'good',scaleText)}${card('Max follow-up depth',investigation.max_followup_depth??4,'muted','Safety cap prevents infinite chaining.')}</section>${button}${debugBlock('Follow-up definitions',investigation.definitions||{})}</section>`;}
