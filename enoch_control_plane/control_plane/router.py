...
def _budget_endpoint_diagnostics(url: str) -> dict[str, str]:
    parsed = urlparse(url)
    return {
        "budget_endpoint_host": parsed.hostname or "",
        "budget_endpoint_path": parsed.path or "",
    }


def _synthetic_budget_proxy_injects_auth(base_url: str) -> bool:
    try:
        hostname = (urlparse(base_url).hostname or "").rstrip(".").lower()
    except ValueError:
        return False
    return hostname == "synthetic.int.exe.xyz"


def _synthetic_budget_request_api_key(base_url: str, api_key: str) -> str:
    return "" if _synthetic_budget_proxy_injects_auth(base_url) else api_key


def _synthetic_budget_auth_mode(base_url: str, api_key: str) -> str:
    if _synthetic_budget_request_api_key(base_url, api_key):
        return "bearer"
    if _synthetic_budget_proxy_injects_auth(base_url):
        return "exe_http_proxy"
    return "none"
...
def _resolve_synthetic_budget_provider(
    ...
    provider_api_key = os.environ.get("ENOCH_RESEARCH_PROVIDER_API_KEY", "")
    if config is not None and not os.environ.get("ENOCH_RESEARCH_PROVIDER_BASE_URL"):
        settings = read_llm_settings(config)
        provider = _llm_provider_by_id(settings, "synthetic")
        base_url = _synthetic_budget_base_url(str(provider.base_url))
        provider_api_key = llm_provider_api_key(config, provider)
    ...
    *,
    ...
    if flags.maintenance_mode and held_override_action != "dispatch-one-while-held":
        raise HTTPException(
            status_code=409,
            detail=(
                "maintenance mode blocks live dispatch; set override_hold_action="
                "dispatch-one-while-held for an explicit operator override"
            ),
        )
    ...
    if (
        ...
        raise HTTPException(
            status_code=409,
            detail=(
                "queue pause blocks live dispatch; set override_hold_action="
                "dispatch-one-while-held for an explicit operator override"
            ),
        )
...
def _post_live_dispatch_run(
    *,
    ...
    dispatch_payload = {
        ...
        "sandbox": str(candidate.get("sandbox") or "workspace-write"),
    }
...
def _execute_live_dispatch(
    ...
    *,
    ...
    _assert_live_dispatch_preconditions(
        ...
        held_override_action=held_override_action,
    )
...
def _select_generation_target_lane(lane_feed_pressure: dict) -> str | None:
    ...
    generation_target_candidates = [
        ...
        if (item.get("machine_target") or not int(item.get("promotable_count") or 0))
        and int(item.get("queue_deficit") or 0) > int(item.get("promotable_count") or 0)
...
def _provider_generation_event_payload(
    *,
    ...
    payload = {
        "status": status,
        "provider": params.provider_id or "synthetic.new",
        "provider_id": params.provider_id or "synthetic",
...
def _fetch_synthetic_research_budget(
    *,
    ...
    budget_endpoint = ""
    budget_base_url = _synthetic_budget_base_url(provider_base_url)
    ...
    if normalized_provider_id not in {"", "env", "synthetic"} and (
        ...
        return {
            ...
            "reserve_requests": bounded_int("reserve_requests", 2, 1, 100),
            "failures": [
                f"provider budget check unavailable for provider {provider_id}"
            ],
            "budget_check_skipped": False,
            "reason": f"provider budget check unavailable for provider {provider_id}",
...
async def _poll_dispatch_completion(
    *,
    store: Any,
...
async def _wait_for_completion(
    *,
    ...
    if max_wait_seconds <= 0:
        ...
        wait_result = await _poll_dispatch_completion(
            store=store,
...
def _append_publication_finalization_stage(
    *,
    response: dict[str, Any],
    finalized_papers: list[dict[str, Any]],
    paper_id: str,
    rewrite_response: Any,
) -> None:
...
def _try_rewrite_paper_review_draft(
    *,
    response: dict[str, Any],
    finalized_papers: list[dict[str, Any]],
    ...
    try:
        ...
        return False
    _append_publication_finalization_stage(
        response=response,
...
def _execute_research_paper_stages(
    *,
    ...
    skip_stage = _paper_draft_skip_stage(
        store=store,
        response=response,
        wait_for_completion=wait_for_completion,
    ...
    if skip_stage:
        response["stages"].append(skip_stage)
        return drafted_papers, finalized_papers
    for draft_index in range(max_paper_drafts):
        ...
        if len(
            finalized_papers
        ) < max_publication_rewrites and not _try_rewrite_paper_review_draft(
            response=response,
            finalized_papers=finalized_papers,
...
async def _execute_live_research_cycle(
    *,
    params: _LiveResearchCycleParams,
    response: dict[str, Any],
) -> dict[str, Any]:
...
def _append_worker_settling_finding(
    *,
    lane_label: str,
    lane_preflight: Any,
    worker_settling: dict[str, Any],
...
def _append_worker_no_live_findings(
    ...
    *,
    ...
    if isinstance(worker_settling, dict):
        _append_worker_settling_finding(
            lane_label=lane_label,
            lane_preflight=lane_preflight,
            worker_settling=worker_settling,
            warnings=warnings,
            blockers=blockers,
        )
        return
    finding = _orphan_worker_lane_finding(lane, lane_label=lane_label, no_live=no_live)
    warnings.append(finding)
    conflicts.append(finding)
    blockers.append(f"worker live run without active control-plane row: {lane_label}")


def _append_worker_confirmation_finding(
    lane: dict[str, Any],
    *,
    lane_label: str,
    warnings: list[DashboardFinding],
    ...
    *,
    ...
    for lane in worker_lanes:
        ...
        if not int(lane.get("active_count") or 0) and no_live:
            _append_worker_no_live_findings(
                lane,
                lane_label=lane_label,
                lane_preflight=lane_preflight,
                no_live=no_live,
            ...
            continue
        _append_worker_confirmation_finding(
            lane,
            lane_label=lane_label,
            warnings=warnings,
            blockers=blockers,
