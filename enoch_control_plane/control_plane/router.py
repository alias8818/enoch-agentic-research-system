...
function workerLaneCards(status={}){const lanes=status.worker_lanes||[]; const cards=lanes.map(l=>{const active=l.active_item||null, next=l.next_candidate||null, tone=l.status==='active'?'info':l.dispatch_available?'good':'warn', line=active?`Current: ${esc(active.project_name||active.project_id||'active project')}`:next?`Next: ${esc(next.project_name||next.project_id||'queued project')}`:esc(l.dispatch_blocker||'No queued candidate for lane'), reason=l.dispatch_available?(l.dispatch_reason||'lane open'):(l.dispatch_blocker||'not dispatchable'); return `<div class="metric-card"><div><div class="label">${esc(workerLaneLabel(l))}</div><div class="value ${tone}">${esc(l.status||'unknown')}</div></div><div class="muted">${line}</div><div class="row"><span class="pill">queued ${esc(l.queued_count||0)}</span><span class="pill ${l.dispatch_available?'good':'warn'}">${esc(reason)}</span></div></div>`;}).join('')||'<div class="empty-state"><div><strong>No worker lanes configured</strong><div>Configure worker targets to expose lane capacity.</div></div></div>'; return `<section class="section-panel" id="workerLanes"><div class="section-head"><div><h2>Worker lanes</h2><div class="section-copy">CPU lane and GB10 lane capacity, queued work by lane, and the reason the next candidate can or cannot dispatch.</div></div></div><section class="metric-grid">${cards}</section></section>`;}
