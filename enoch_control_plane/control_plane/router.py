...
function heroStatusCard(state,counts={},papers={},generatedAt='',ops={}){const tone=state?.tone||'info', label=esc(state?.label||'System ready'), detail=esc(state?.detail||'No active or queued work right now.'); const fresh=esc(formatRelativeAge(generatedAt))||'just now'; const active=Number(counts.active||0), queued=Number(counts.queued||0), draftsAll=Number(papers.publication_draft||0)+Number(papers.draft_review||0), publishedImported=Number(ops.published||0); return `<section class="hero-status banner ${tone}"><div class="hero-status__inner"><div class="hero-status__lead"><div class="hero-status__label">System status</div><h2 class="hero-status__title">${label}</h2><div class="hero-status__detail">${detail}</div><div class="hero-status__meta"><span class="pill ${tone}">${esc((state?.tone||'info').toUpperCase())}</span><span class="pill">Last refreshed ${fresh}</span><span class="pill">Bounded read models</span></div></div><div class="kpi-strip"><div class="kpi"><div class="kpi-label">Active</div><div class="kpi-value ${active?'info':'good'}">${esc(active)}</div></div><div class="kpi"><div class="kpi-label">Queued</div><div class="kpi-value ${queued?'info':'good'}">${esc(queued)}</div></div><div class="kpi"><div class="kpi-label">Drafts</div><div class="kpi-value ${draftsAll?'info':'good'}">${esc(draftsAll)}</div></div><div class="kpi"><div class="kpi-label">Published</div><div class="kpi-value good">${esc(publishedImported)}</div></div></div></div></section>`;}
function actionToneClass(t){const v=String(t||'').toLowerCase(); return ['warn','bad','critical','info','good'].includes(v)?v:'info';}
function topActionsCard(actions=[],state={}){if(!actions.length){const idle=String(state?.tone||'good')==='good'; return `<div class="action-empty"><div class="check" aria-hidden="true">✓</div><div><strong>${idle?'All clear':'Nothing ranked right now'}</strong><div class="muted">${idle?'Nothing needs operator action. Drill into the breakdown below for the full state.':'No bounded actions are ranked above the operator threshold. Check active work and recent activity for context.'}</div></div></div>`;} return `<div class="action-stack">${actions.map((a,i)=>{const tone=actionToneClass(a.tone), title=esc(a.title||'Action'), summary=esc(a.summary||''), label=esc(a.action_label||'Open'), hash=esc(a.action_hash||'#overview'), rank=esc(i+1); return `<div class="action-row ${tone}"><div class="action-row__rank" aria-hidden="true">${rank}</div><div class="action-row__body"><div class="action-row__title">${title}</div><div class="action-row__summary">${summary}</div></div><div class="action-row__cta"><a class="button-primary" href="${hash}">${label}</a></div></div>`;}).join('')}</div>`;}
function pipelineStepperCard(pipeline={}){const write=Number(pipeline.write_needed||0), finalize=Number(pipeline.finalize_needed||0), publish=Number(pipeline.publish_ready||0); const nextWrite=pipeline.next_write_candidate||null, nextPublish=pipeline.next_publish_candidate||null; const writeDetail=write?`Next: ${esc(nextWrite?.project_name||nextWrite?.project_id||'untitled')}`:'No paper-ready run is awaiting a draft.'; const finalizeDetail=finalize?'Drafts pending automation finalization.':'No drafts pending finalization.'; const publishDetail=publish?`Next: ${esc(nextPublish?.project_name||nextPublish?.paper_id||'untitled')}`:'No finalized drafts pending corpus import.'; const tone=v=>v?'warn':'good'; return `<div class="stepper-modern"><div class="stepper-modern__step ${tone(write)}"><div class="step-index" data-step="1">Write</div><div class="step-value">${esc(write)}</div><div class="step-detail">${writeDetail}</div></div><div class="stepper-modern__step ${tone(finalize)}"><div class="step-index" data-step="2">Finalize</div><div class="step-value">${esc(finalize)}</div><div class="step-detail">${finalizeDetail}</div></div><div class="stepper-modern__step ${tone(publish)}"><div class="step-index" data-step="3">Publish</div><div class="step-value">${esc(publish)}</div><div class="step-detail">${publishDetail}</div></div></div>`;}

async function overviewPage(){const routeKey=(location.hash||'#overview').slice(1).split('?')[0]||'overview'; renderNav('overview'); const app=$('app'), hasOverview=app?.dataset?.page==='overview'; const previousCommandPanel=hasOverview?$('commandPanel')?.outerHTML||'':''; $('status').className='pill warn'; $('status').textContent=hasOverview?'Refreshing overview…':'Loading overview…'; if(!hasOverview){app.className=''; app.innerHTML='<section class="hero-status"><div class="hero-status__inner"><div class="hero-status__lead"><div class="hero-status__label">Loading status</div><h2 class="hero-status__title">Fetching the bounded operator summary…</h2><div class="hero-status__detail">Reading bounded read models. Raw states stay in drill-down views.</div></div><div class="kpi-strip"><div class="skeleton" style="min-height:64px"></div><div class="skeleton" style="min-height:64px"></div><div class="skeleton" style="min-height:64px"></div><div class="skeleton" style="min-height:64px"></div></div></div></section><section class="section-panel"><div class="section-head"><div><h2>Loading overview…</h2><div class="section-copy">Fetching the bounded operator summary first.</div></div></div><section class="metric-grid"><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div></section></section>';} const overview=await api('/control/api/v1/overview?active_limit=8&event_limit=6'); if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const counts=overview.counts||{}, papers=overview.paper_counts||{}, ops=overview.operator_counts||{}, pipeline=overview.paper_pipeline||{}, investigation=overview.investigation_pipeline||{}, topActions=Array.isArray(overview.top_actions)?overview.top_actions:[], state=workState(counts,ops,pipeline,investigation); $('status').className='pill '+state.tone; $('status').textContent=`Overview · ${state.label}`; setSystemState(state.tone,state.label); $('app').dataset.page='overview'; $('app').className=''; $('app').innerHTML=`${heroStatusCard(state,counts,papers,overview.generated_at,ops)}<section class="section-panel"><div class="section-head"><div><h2>What needs me right now?</h2><div class="section-copy">Top ranked actions, computed server-side from bounded operator and pipeline counts. Empty when the system has nothing for you.</div></div></div>${topActionsCard(topActions,state)}</section><section class="section-panel" id="workerLanes"><div class="section-head"><div><h2>Worker lanes</h2><div class="section-copy">Loading CPU lane and GB10 lane capacity…</div></div></div><section class="metric-grid"><div class="skeleton"></div><div class="skeleton"></div></section></section><section class="section-panel"><div class="section-head"><div><h2>Paper pipeline</h2><div class="section-copy">Write → finalize → publish. Operator-actionable counts only; the full breakdown stays in the collapsed detail panel below.</div></div></div>${pipelineStepperCard(pipeline)}</section><section class="section-panel" id="automationReadiness"><div class="section-head"><div><h2>Automation readiness</h2><div class="section-copy">Checking queue flags, timers, recent ticks, provider budget, and paper gate status.</div></div></div><section class="metric-grid"><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div></section></section>${previousCommandPanel||operatorCommandPanel(counts)}<details id="overviewSecondary" class="section-panel overview-secondary"><summary>Active work, system health, and recent activity</summary><div class="muted">Secondary status. Useful but should not compete with the action model above.</div><section class="grid two"><div class="card"><h2>Active work</h2><div class="muted">Only work currently running or awaiting callback appears here.</div>${tableRows(overview.active_items||[],['operator_stage_label','project_id','current_run_id','operator_next_step','updated_at'],'No active work right now')}</div><div class="card" id="overviewHealth"><h2>System health</h2><div class="skeleton"></div><div class="muted">Secondary health checks load after the primary cards render so they do not block the overview.</div></div></section><section class="card"><h2>Recent activity</h2><div class="muted">Formatted control-plane events. Large payloads stay hidden.</div>${activityCards(overview.recent_events||[])}</section></details><details id="operatorDetailBreakdown" class="section-panel"><summary>Operator detail breakdown</summary><div class="muted">Full question-led counts, investigation pipeline, and paper-pipeline detail. Kept here for drill-down — the top-of-fold cards above are the primary view.</div>${operatorQuestionCards(counts,ops,pipeline,investigation)}${investigationPipelineCards(investigation)}${paperPipelineCards(pipeline,papers)}</details>`; api('/control/api/status').then(status=>{if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('workerLanes'); if(el)el.outerHTML=workerLaneCards(status);}).catch(e=>{if(e.name==='AbortError')return; if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('workerLanes'); if(el)el.innerHTML=`<h2>Worker lanes</h2><span class="pill warn">Lane capacity unavailable</span><div class="muted">${esc(e.message)}</div>`;}); api('/control/api/v1/automation-readiness').then(readiness=>{if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('automationReadiness'); if(el)el.outerHTML=automationReadinessCard(readiness);}).catch(e=>{if(e.name==='AbortError')return; if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('automationReadiness'); if(el)el.innerHTML=`<h2>Automation readiness</h2><span class="pill warn">Readiness unavailable</span><div class="muted">${esc(e.message)}</div>`;}); Promise.all([api('/control/api/v1/observability/memory'),api('/control/api/v1/observability/health')]).then(([memory,health])=>{if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('overviewHealth'); if(!el)return; el.innerHTML=`<h2>System health</h2><div class="row"><span class="pill ${memory.memory_warn?'warn':'good'}">Controller memory ${esc(Number(memory.rss_mib||0).toFixed(0))} MiB</span><span class="pill">Peak ${esc(Number(memory.peak_rss_mib||0).toFixed(0))} MiB</span><span class="pill ${health.route_observability_enabled?'good':'warn'}">Route observations ${health.route_observability_enabled?'on':'off'}</span></div><div class="muted">Memory is shown as a secondary health signal, not the primary work status.</div>`;}).catch(e=>{if(e.name==='AbortError')return; if((location.hash||'#overview').slice(1).split('?')[0]!==routeKey)return; const el=$('overviewHealth'); if(el)el.innerHTML=`<h2>System health</h2><span class="pill warn">Health unavailable</span><div class="muted">${esc(e.message)}</div>`;}); refreshCommandPanel();}
