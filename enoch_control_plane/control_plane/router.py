...
function automationReadinessCard(readiness={}){const ok=!!readiness.ok, label=readiness.label||'Long-haul mode: UNKNOWN', blockers=readiness.blockers||[], summary=readiness.summary||{}, checks=readiness.checks||[]; const tone=ok?'good':'warn'; const blockerHtml=blockers.length?`<ul>${blockers.slice(0,6).map(b=>`<li>${esc(b)}</li>`).join('')}</ul>`:'<div class="muted">All long-haul readiness checks passed.</div>'; const checkHtml=checks.slice(0,8).map(c=>`<span class="pill ${c.ok?'good':'warn'}">${esc(c.name)}: ${c.ok?'ok':'blocked'}</span>`).join(''); return `<section class="card" id="automationReadiness"><h2>Automation readiness</h2><div class="row"><span class="pill ${tone}">${esc(label)}</span></div><div class="muted">Single source for overnight/24x7 readiness. This must be READY before claiming unattended long-haul operation.</div>${blockerHtml}<div class="row">${checkHtml}</div><section class="grid">${card('Queue',`queued ${summary.queued||0} · active ${summary.active||0}`,tone,`paused=${summary.queue_paused?'true':'false'} · maintenance=${summary.maintenance_mode?'true':'false'}`)}${card('Paper lane',`write ${summary.write_needed||0} · publish ${summary.publish_ready||0}`,tone,`${summary.published_imported||0} imported`)}</section></section>`;}
