...
def evaluate_longhaul_readiness(
    *,
    ...
    research_active = (
        str(
            research_timer.get("ActiveState")
            or research_timer.get("active_state")
            or ""
        )
        == "active"
    )
    corpus_active = (
        str(corpus_timer.get("ActiveState") or corpus_timer.get("active_state") or "")
        == "active"
    )
    add(
        check(
    ...
    research_result = str(
        research_service.get("Result") or research_service.get("result") or ""
    )
    corpus_result = str(
        corpus_service.get("Result") or corpus_service.get("result") or ""
    )
    add(
        check(
    ...
    if queued == 0:
        queued_state_consistent = True
    elif bool(next_candidate):
        queued_state_consistent = True
    elif open_lane_has_queued_work:
    ...
    return {
        ...
        if status == "ready"
        else f"Long-haul mode: BLOCKED — {blockers[0]}",
        "blockers": blockers,
        "checks": checks,
        ...
        "summary": {
            ...
            "source_lineage_followups_checked": int(
                lineage.get("followups_checked") or 0
            ),
