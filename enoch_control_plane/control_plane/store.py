...
def _reject_conflicting_snapshot_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        if not key:
            continue
        identity = tuple(_text(_first_present(row, *fields)) for fields in identity_fields)
        ...
        with self._connect() as conn:
            rows = conn.execute(
                """SELECT q.*,
                    ...
                    (SELECT pa.draft_markdown_path FROM papers pa WHERE pa.project_id = q.project_id AND (q.current_run_id = '' OR pa.run_id = q.current_run_id) ORDER BY pa.updated_at DESC LIMIT 1) AS related_draft_markdown_path,
                    (SELECT pa.evidence_bundle_path FROM papers pa WHERE pa.project_id = q.project_id AND (q.current_run_id = '' OR pa.run_id = q.current_run_id) ORDER BY pa.updated_at DESC LIMIT 1) AS related_evidence_bundle_path,
                    (SELECT pa.claim_ledger_path FROM papers pa WHERE pa.project_id = q.project_id AND (q.current_run_id = '' OR pa.run_id = q.current_run_id) ORDER BY pa.updated_at DESC LIMIT 1) AS related_claim_ledger_path,
                    (SELECT pa.manifest_path FROM papers pa WHERE pa.project_id = q.project_id AND (q.current_run_id = '' OR pa.run_id = q.current_run_id) ORDER BY pa.updated_at DESC LIMIT 1) AS related_manifest_path,
