...
from scripts.agentic_property_testing import (
    execute_proposals,
    generate_provider_proposal,
    run_autonomous_loop,
    synthetic_budget_preflight,
    write_prompt,
)
...
def test_agentic_property_testing_classifies_collection_errors_as_proposal_error(
    tmp_path: Path,
) -> None:
    repo = tmp_path
    proposals = repo / "proposals.json"
...
def test_agentic_property_testing_classifies_pytest_invocation_errors() -> None:
    assert (
        agentic_property_testing._execution_status(
            4, "ERROR: file or directory not found: -q"
        )
        == "execution_error"
    )
    assert (
...
def test_agentic_property_testing_does_not_hide_counterexamples_with_fixture_word() -> (
    None
):
    output = "E   AssertionError: fixture behavior violated for generated input"
    assert (
        agentic_property_testing._execution_status(1, output) == "counterexample_found"
    )
    assert (
        agentic_property_testing._execution_status(
            1, "E   fixture 'missing_fixture' not found"
        )
        == "proposal_error"
    )


def test_agentic_property_testing_proposal_metadata_cannot_break_out_of_comments(
    tmp_path: Path,
) -> None:
...
def test_agentic_property_testing_main_strips_passthrough_separator(
                ...
                "tests": [
                    {
                        "name": "identity",
                        "rationale": "identity",
                        "code": "from sample_module import identity\n\ndef test_identity():\n    assert identity('x') == 'x'\n",
                    }
                ]
            }
        ),
...
def test_agentic_property_testing_autonomous_loop_retries_proposal_errors(
    tmp_path: Path,
) -> None:
    repo = tmp_path
                ...
                "tests": [
                    {
                        ...
                        "rationale": "identity returns input",
    ...
    result = run_autonomous_loop(
        repo, module, [invalid, valid], repo / "reports", max_attempts=3
    )
...
def test_agentic_property_testing_autonomous_loop_max_attempt_exhaustion(
    tmp_path: Path,
) -> None:
    repo = tmp_path
    target = repo / "sample.py"
    ...
    result = run_autonomous_loop(
        repo, target, [invalid], repo / "reports", max_attempts=1
    )
...
def test_agentic_property_testing_synthetic_budget_preflight_supports_no_auth_proxy(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        agentic_property_testing.research_provider_budget, "fetch_json", fake_fetch_json
    )
...
def test_agentic_property_testing_provider_proposal_writes_json(
    ...
    class FakeResponse:
        ...
        def read(self) -> bytes:
            ...
            return json.dumps(
                {"id": "resp-1", "choices": [{"message": {"content": content}}]}
            ).encode()
    ...
    assert (
        json.loads(output.read_text(encoding="utf-8"))["tests"][0]["name"] == "sample"
    )
    assert "Authorization" not in seen_headers


def test_agentic_property_testing_provider_proposal_rejects_unsafe_url(
    monkeypatch, tmp_path: Path
) -> None:
...
def test_agentic_property_testing_provider_budget_failure_fails_closed(
    ...
    monkeypatch.setattr(
        agentic_property_testing, "synthetic_budget_preflight", fail_budget
    )
...
def test_agentic_property_testing_provider_autonomous_loop_requires_review(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    target = tmp_path / "sample.py"
    target.write_text("def identity(value):\n    return value\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: {
            "ok": True,
            "proposal_file": str(tmp_path / "provider-proposal.json"),
            "model": "m",
            "response_id": "r",
        },
    )
...
def test_agentic_property_testing_provider_generation_failure_fails_closed(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    target = tmp_path / "sample.py"
    target.write_text("def identity(value):\n    return value\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        agentic_property_testing, "generate_provider_proposal", fail_generate
    )
