...
def _write_evalset(path: Path) -> None:
    rows = [
        {"schema_version": "enoch_research_quality_evalcase_v1", "case_id": "duplicateish_candidate:a", "case_type": "duplicateish_candidate"},
        {"schema_version": "enoch_research_quality_evalcase_v1", "case_id": "proxy_only_positive:b", "case_type": "proxy_only_positive"},
        {"schema_version": "enoch_research_quality_evalcase_v1", "case_id": "supported_but_negative_warning:c", "case_type": "supported_but_negative_warning"},
        {"schema_version": "enoch_research_quality_evalcase_v1", "case_id": "max_depth_followup_ending:d", "case_type": "max_depth_followup_ending"},
        {"schema_version": "enoch_research_quality_evalcase_v1", "case_id": "useful_adjacent_followup:e", "case_type": "useful_adjacent_followup"},
    ]
...
def test_evolve_research_prompt_outputs_patch_only_artifacts(tmp_path: Path) -> None:
    ...
    report = json.loads((output_dir / "evolution_report.json").read_text(encoding="utf-8"))
    patch = (output_dir / "research_provider_prompt.patch").read_text(encoding="utf-8")
    prompt = (output_dir / "research_provider_prompt_candidate.md").read_text(encoding="utf-8")
...
def test_evolve_research_prompt_rejects_unknown_target(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "out"
    _write_evalset(evalset)
    source.write_text("Avoid fake citations. Avoid tiny hyperparameter or +0.05% ideas.\n", encoding="utf-8")
    try:
        evolve_research_prompt.main(["--evalset", str(evalset), "--source", str(source), "--output-dir", str(output_dir), "--target", "worker-system-prompt"])
