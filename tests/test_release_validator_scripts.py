...
def test_reject_stale_matches_phrase_case_insensitively() -> None:
    ...
    validate_live_public_release.reject_stale("launch", "This page says 120 AI-generated artifacts.", failures)

    assert failures == ["launch contains stale/unscoped phrase: 120 AI-generated"]


def test_live_public_validation_defaults_expected_count_from_manifest(monkeypatch) -> None:
    manifest = {
        "artifact_count": 388,
    ...
    hf_summary = {"artifact_count": 388, "strict_claim_evidence_pass_count": 3, "strict_claim_evidence_total_count": 388}
    ...
    assert validate_live_public_release.main([
        "--launch", "https://launch",
        "--profile", "https://profile",
        "--manifest", "https://manifest",
        "--docs", "https://docs",
        "--hf-summary", "https://hf-summary",
        "--hf-readme", "https://hf-readme",
    ]) == 0
...
def _write_manifest_inputs(corpus: Path) -> None:
    ...
    (corpus / "papers" / "index.json").write_text(json.dumps({"count": 1, "papers": [{}]}), encoding="utf-8")
    (corpus / "quality" / "quality_report.json").write_text(json.dumps({"count": 1, "passed": 1}), encoding="utf-8")


def test_generate_ecosystem_manifest_rejects_missing_system_and_docs_repos(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--system", str(tmp_path / "missing-system"),
        "--docs", str(tmp_path / "missing-docs"),
        "--corpus", str(corpus),
        "--output", str(output),
    ])
