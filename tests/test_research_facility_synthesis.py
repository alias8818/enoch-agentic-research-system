...
def test_cluster_gate_collapses_related_spec_decoding_branches() -> None:
    ...
    assert (
        clusters[0]["cluster_key"] == "spec-decoding:https://arxiv.org/abs/2603.03251"
    )
    assert {row["candidate_id"] for row in clusters[0]["candidates"]} == {
        f"spec-branch-{i}" for i in range(1, 6)
    }
...
def test_unrelated_candidates_do_not_trigger_synthesis() -> None:
    rows = [
        _candidate(
            ...
            source_urls=["https://example.com/kv"],
            mechanism="kv cache",
        ),
        _candidate(
            2,
...
def test_reflection_patterns_use_successes_as_seed_context_not_truth() -> None:
    patterns = synth.extract_reflection_patterns(
        [
            ...
            {
                "project_id": "negative",
                "decision_gate_state": "finalize_negative",
                "decision_summary": "junk",
            },
        ]
    )
...
def test_validation_accepts_artifact_description_maps_from_provider() -> None:
    candidate = {
        ...
        "expected_artifacts": {
            artifact: "description" for artifact in synth.REQUIRED_ORACLE_ARTIFACTS
        },
        "required_evidence": ["decision matrix"],
        "likely_failure_modes": ["no signal"],
        "novelty_score": 9,
...
def test_validation_rejects_provider_candidate_that_would_fail_admission_contract() -> (
    None
):
    candidate = {
...
def test_provider_synthesis_builds_one_valid_meta_candidate() -> None:
    ...
    assert candidate["raw_candidate_json"]["synthesized_from"] == [
        f"spec-branch-{i}" for i in range(1, 6)
    ]
...
def test_enrichment_normalizes_provider_artifact_maps_to_arrays() -> None:
    ...
    assert sorted(candidate["expected_artifacts"]) == sorted(
        synth.REQUIRED_ORACLE_ARTIFACTS
    )
...
def test_synthesis_sql_defers_branches_and_records_lineage() -> None:
    ...
    report = {
        ...
        "synthesized_candidates": [
            synth.enrich_synthesized_candidate(
                synthesized, cluster, [], requested_by="unit"
            )
        ],
    }
...
def test_synthesis_sql_refuses_candidates_that_do_not_pass_admission() -> None:
    ...
    with pytest.raises(ValueError, match="must pass admission"):
        synth.emit_synthesis_sql(
            {"clusters": [cluster], "synthesized_candidates": [rejected_candidate]},
            requested_by="unit",
            queue_synthesized=True,
        )
