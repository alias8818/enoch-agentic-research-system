...
def test_config_token_does_not_follow_env_control_url(monkeypatch, tmp_path):
    ...
    config.write_text(
        json.dumps({"listen_host": "0.0.0.0", "listen_port": 8787, "control_api_bearer_token": "config-token"}),
        encoding="utf-8",
    )
    observed = {}
...
def test_explicit_token_may_use_env_control_url(monkeypatch, tmp_path):
    config = tmp_path / "config.json"
    config.write_text(json.dumps({"listen_host": "0.0.0.0", "listen_port": 8787}), encoding="utf-8")
    observed = {}
    ...
    assert check_longhaul_readiness.main(["--live", "--config", str(config), "--token", "explicit-token"]) == 0
    assert observed == {
