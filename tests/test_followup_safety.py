...
from enoch_control_plane.control_plane.supabase_store import _enforced_followup_depth, _followup_escalation_payload
...
def test_promising_negative_followup_escalates_beyond_proxy() -> None:
    payload = _followup_escalation_payload({
        ...
        "followup_required_evidence": ["direct baseline comparison", "fixed-seed ablation"],
...
def test_worker_prompt_includes_controller_escalation_ladder() -> None:
    prompt = _project_prompt({
        ...
        "idea_source_payload_json": {
            ...
            "worker_prompt_guidance": ["Use direct target metrics and a real baseline."],
        },
    })
