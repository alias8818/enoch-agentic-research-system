...
def test_research_facility_scan_turns_source_json_into_candidate_batch(tmp_path: Path) -> None:
    ...
    assert research_facility_scan.main(["--source-json", str(source), "--output", str(output)]) == 0
...
def test_research_facility_scan_parses_arxiv_atom_without_network(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(research_facility_scan, "fetch_text", lambda url, timeout=20: atom)

    records = research_facility_scan.scan_arxiv("all:speculative decoding", max_results=1, timeout=1)
    ...
    candidate = research_facility_scan.candidate_from_source(records[0], default_machine="worker", default_model="model", default_sandbox="sandbox")
    assert candidate["category"] == "spec-decoding"
    assert candidate["source_ids"] == [records[0].source_id]


def test_research_facility_scan_records_fetch_errors_without_losing_batch(monkeypatch, tmp_path: Path) -> None:
    output = tmp_path / "batch.json"
    monkeypatch.setattr(research_facility_scan, "fetch_text", lambda url, timeout=20: (_ for _ in ()).throw(RuntimeError("rate limited")))

    assert research_facility_scan.main(["--arxiv-query", "all:test", "--output", str(output)]) == 0
