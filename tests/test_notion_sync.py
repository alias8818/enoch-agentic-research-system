    ...
    def __call__(
        ...
        if url.endswith("/databases/db"):
            return HttpResponse(
                ...
                body={
                    "object": "database",
                    "data_sources": [{"id": "ds-1", "name": "Ideas"}],
                },
            )
                    ...
                    "results": [
                        {
                            ...
                            "properties": {
                                "Idea": {
                                    "type": "title",
                                    "title": [{"plain_text": "Idea One"}],
                                },
                                "Status": {
                                    "type": "select",
                                    "select": {"name": "exploring"},
                                },
                                "Priority": {
                                    "type": "select",
                                    "select": {"name": "High"},
        ...
        if url.endswith("/control/intake/notion-ideas"):
            ...
            return HttpResponse(
                ...
                body={
                    ...
                    "candidates": payload["notion_rows"],
                    "created": 0,
                },
            )
                    ...
                    "rows": [
                        {
                            ...
                            "properties": {
                                ...
                                "Current Run ID": "",
                                "Next Action": "controller_review",
                                "Blocked Reason": "",
                                "Last Execution Update": "2026-04-28T00:00:00Z",
                                "Execution Summary": "",
                            },
                        }
                    ],
...
class NotionSyncTests(unittest.TestCase):
    def test_cli_main_is_disabled_unless_legacy_sync_is_explicitly_enabled(
        self,
    ) -> None:
    ...
    def test_update_payload_uses_safe_notion_property_shapes(self) -> None:
        payload = notion_update_properties(
            {
                "properties": {
                    "Execution State": "queued",
                    "Current Run ID": "run-1",
                    "Next Action": "x",
                    "Blocked Reason": "",
                    "Last Execution Update": "2026-04-28T00:00:00Z",
                    "Execution Summary": "summary",
                    "Enoch Project ID": "idea-1",
                    "Enoch Queue Status": "queued",
                    "Enoch Manual Review Required": "__YES__",
                    "Enoch Dispatch Priority": 7,
                    "Enoch Paper Updated At": "2026-04-28T01:00:00Z",
                }
            }
        )
        self.assertEqual(payload["Execution State"], {"select": {"name": "queued"}})
        self.assertEqual(
            payload["Current Run ID"]["rich_text"][0]["text"]["content"], "run-1"
        )
        self.assertEqual(
            payload["Last Execution Update"],
            {"date": {"start": "2026-04-28T00:00:00Z"}},
        )
        self.assertEqual(
            payload["Enoch Project ID"]["rich_text"][0]["text"]["content"], "idea-1"
        )
        ...
        self.assertEqual(
            payload["Enoch Paper Updated At"],
            {"date": {"start": "2026-04-28T01:00:00Z"}},
        )
    ...
    def test_apply_execution_updates_skips_when_page_has_no_supported_properties(
        self,
    ) -> None:
        def transport(
            ...
            [
                {
                    "page_id": "page-empty",
                    "project_id": "p1",
                    "properties": {"Execution State": "queued"},
                }
            ],
            "secret",
            transport=transport,
    ...
    def test_runner_defaults_to_dry_run_without_writes(self) -> None:
        ...
        intake_payload = next(
            ...
            if call[1].endswith("/control/intake/notion-ideas")
        )
