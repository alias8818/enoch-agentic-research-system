    ...
    def __init__(self, root: Path) -> None:
        ...
        self.records = [
            RunRecord(
                ...
                project_dir=str(root / "project-a"),
            ),
            RunRecord(
                run_id="run-ready",
...
def _client(tmp_path: Path, monkeypatch) -> TestClient:
    ...
    fake_store.events_log.write_text(
        json.dumps({"kind": "event", "run_id": "run-live", "payload": "x" * 2000})
        + "\n"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda: TelemetrySample(
            cpu_pct=1, gpu_pct=2, memory_source="uma_meminfo", uma_allocatable_mib=1234
        ),
    )
...
def test_dashboard_api_and_run_detail_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    (tmp_path / "state" / "paper_snapshot.json").write_text(
        json.dumps({"total": 2, "rows": []})
    )
    ...
    assert (
        data["service"]["completion_callback_token_fingerprint"]
        == hashlib.sha256(
            appmod.config.completion_callback_token.encode("utf-8")
        ).hexdigest()
    )
    ...
    run = client.get(
        "/dashboard/api/run/run-live", headers={"Authorization": f"Bearer {token}"}
    )
    assert run.status_code == 200
    assert run.json()["run"]["run_id"] == "run-live"
    assert (
        client.get(
            "/dashboard/api/run/missing", headers={"Authorization": f"Bearer {token}"}
        ).status_code
        == 404
    )


def test_dashboard_run_detail_treats_unexpandable_project_dir_as_unreadable(
    ...
    run = client.get(
        "/dashboard/api/run/run-unexpandable",
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_dashboard_run_detail_does_not_read_project_dir_outside_root(
    tmp_path: Path, monkeypatch
) -> None:
    client, token = _client(tmp_path, monkeypatch)
...
def test_dashboard_snapshot_writes_preserve_existing_files_on_replace_failure(
    tmp_path: Path, monkeypatch
) -> None:
    client, token = _client(tmp_path, monkeypatch)
    headers = {"Authorization": f"Bearer {token}"}
    ...
    queue_response = failing.post(
        "/dashboard/queue-snapshot", headers=headers, json={"rows": [], "total": 0}
    )
    paper_response = failing.post(
        "/dashboard/paper-snapshot", headers=headers, json={"rows": [], "total": 0}
    )
...
def test_prepare_project_status_and_paper_artifact_endpoints(
    tmp_path: Path, monkeypatch
) -> None:
    client, token = _client(tmp_path, monkeypatch)
    headers = {"Authorization": f"Bearer {token}"}
    ...
    (project_dir / ".enoch" / "project_decision.json").write_text(
        json.dumps({"project_decision": "continue"})
    )
    ...
    written = client.post(
        ...
        json={
            "run_id": "run-live",
            "paper_id": "paper-1",
            "files": [{"path": "papers/run-live/draft.md", "content": "# Draft"}],
            "overwrite": False,
        },
    )
    ...
    escaped_manifest = client.post(
        "/project-paper/project-a",
        headers=headers,
        json={
            "run_id": "../escape",
            "paper_id": "paper-escape",
            "files": [],
            "overwrite": True,
        },
    )
    ...
    read = client.post(
        ...
        headers=headers,
        json={"paths": ["papers/run-live/draft.md"], "max_bytes_per_file": 1000},
    )
...
def test_project_status_rejects_uninspectable_project_dir_without_raw_error(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    response = client.get(
        "/project-status/project-a", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_write_project_paper_rejects_uninspectable_project_dir_without_raw_error(
    tmp_path: Path, monkeypatch
) -> None:
    _, token = _client(tmp_path, monkeypatch)
    project = tmp_path / "project-a"
...
def test_dispatch_endpoint_handles_success_and_bad_output(
    tmp_path: Path, monkeypatch
) -> None:
    client, token = _client(tmp_path, monkeypatch)
    headers = {"Authorization": f"Bearer {token}"}
    ...
    (project / ".enoch" / "project.json").write_text(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            returncode=0, stdout=json.dumps({"pid": 123, "pgid": 456}), stderr=""
        ),
    )
    response = client.post(
        "/dispatch",
...
def test_dispatch_rejects_unsafe_run_id_and_log_dir_escape(
    tmp_path: Path, monkeypatch
) -> None:
    client, token = _client(tmp_path, monkeypatch)
    headers = {"Authorization": f"Bearer {token}"}
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda *args, **kwargs: SimpleNamespace(
            returncode=0, stdout=json.dumps({"pid": 1, "pgid": 1}), stderr=""
        ),
    )
...
def test_callback_outbox_replay_and_reaper_async_helpers(
    ...
    fake_store = appmod.store
    results = [
        callback_outbox.DeliveryResult(ok=True, status_code=200, detail="ok", path="p")
    ]
    monkeypatch.setattr(
        appmod.callback_outbox, "replay_pending", lambda **kwargs: results
    )
...
def test_evaluate_until_ready_retry_timeout_and_callback_paths(
    tmp_path: Path, monkeypatch
) -> None:
    _client(tmp_path, monkeypatch)
    fake_store = appmod.store
    ...
    # Timeout path.
    timeout_record = RunRecord(
        run_id="timeout",
        session_id="s",
        project_id="p",
        gate_state=GateState.PENDING_IDLE_GATE,
        idle_seen_at="2026-01-01T00:00:00+00:00",
        last_event_at="seen",
    )
    ...
    callback_record = RunRecord(
        ...
        idle_seen_at="idle",
        last_event_at="idle",
        last_event=SourceEvent.SESSION_IDLE,
    )
    ...
    monkeypatch.setattr(
        ...
        "evaluate",
        lambda record: (
            record.model_copy(update={"gate_state": GateState.WAKE_READY}),
            gate_callback,
        ),
    )
    monkeypatch.setattr(appmod, "_deliver_callback", _ok)
    ...
    assert (
        fake_store.saved["callback"].last_idempotency_key == "callback:wake_ready:idle"
    )
...
def test_misc_endpoint_error_branches(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        appmod.config, "dispatch_script_path", str(tmp_path / "missing-dispatch.sh")
    )
    missing_script = client.post(
        "/dispatch",
        headers=headers,
        json={
            "run_id": "run-live",
    ...
    assert (
        client.post(
            "/project-paper/project-a/read", headers=headers, json={"paths": ["x"] * 21}
        ).status_code
        == 400
    )
    assert (
    ...
    assert (
        client.post(
            "/project-paper/project-a/read",
            headers=headers,
            json={"paths": ["big.md"], "max_bytes_per_file": 1},
        ).status_code
        == 413
    )
    ...
    assert (
        client.post(
            "/project-paper/missing",
            headers=headers,
            json={"run_id": "run", "paper_id": "paper", "files": []},
        ).status_code
        == 404
    )
    assert (
        client.post(
            ...
            json={
                "run_id": "run",
                "paper_id": "paper",
                "files": [{"path": f"p{i}.md", "content": "x"} for i in range(21)],
            },
        ).status_code
        == 400
    )
    assert (
        client.post(
            ...
            json={
                "run_id": "run",
                "paper_id": "paper",
                "files": [{"path": "too-big.md", "content": "x" * 2_000_001}],
            },
        ).status_code
        == 413
    )
...
def test_paper_artifact_endpoints_reject_uninspectable_project_dir_without_raw_error(
    tmp_path: Path, monkeypatch
) -> None:
    _, token = _client(tmp_path, monkeypatch)
    project = tmp_path / "project-a"
