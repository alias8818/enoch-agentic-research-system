...
def _isolate_sentry_env(monkeypatch):
    """Clear Sentry-related env vars around every test in this module."""
...
def _reload_error_tracking(**env: str):
    """Reload the module after applying the given Sentry env vars."""
    import os
...
def test_capture_exception_without_sentry_logs_and_returns_none(caplog) -> None:
    ...
    records = [
        record for record in caplog.records if record.name == "enoch.error_tracking"
    ]
...
def test_capture_exception_with_sentry_initialized_invokes_sdk_once() -> None:
    error_tracking = _reload_error_tracking()
...
def test_capture_exception_with_sentry_does_not_raise_on_sdk_failure(caplog) -> None:
    error_tracking = _reload_error_tracking()
...
def test_no_sentry_path_does_not_initialize_sdk_or_touch_sdk_calls() -> None:
    error_tracking = _reload_error_tracking()
...
def test_before_send_is_invoked_with_scrubbed_event() -> None:
    error_tracking = _reload_error_tracking()
    event = {
        "request": {
