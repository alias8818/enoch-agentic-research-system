...
class WorkerPreflightTests(unittest.TestCase):
    ...
    def test_preflight_fetches_enough_dashboard_rows_to_classify_gb10_stale_gate(
        self,
    ) -> None:
        seen_urls: list[str] = []
            ...
            if "/dashboard/api" in url:
                return HttpResult(
                    ...
                    body={
                        ...
                        "totals": {"active_or_waiting": 93, "live": 93},
                        "queue": {"active_count": 0},
                        "runs": [
                            {
                                "run_id": f"stale-run-{index}",
                                "project_id": f"project-stale-{index}",
                                "gate_state": "running",
                                "lifecycle_state": "active",
                                "is_live": True,
                                "active_process_count": 0,
        ...
        dashboard_urls = [url for url in seen_urls if "/dashboard/api" in url]
        self.assertEqual(len(dashboard_urls), 1)
        self.assertIn("limit=100", dashboard_urls[0])
        checks = {check.name: check for check in response.checks}
        self.assertTrue(response.ok)
        self.assertTrue(checks["worker_no_live_runs"].data["stale_worker_gate_only"])
        self.assertEqual(checks["worker_no_live_runs"].data["effective_live"], 0)
