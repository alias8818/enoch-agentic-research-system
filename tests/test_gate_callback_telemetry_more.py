...
from enoch_control_plane.process_tracker import (
    ProcessTracker,
    _is_benign_project_process,
)
...
def test_apply_event_precedence_and_new_session_reset() -> None:
    ...
    updated = gate.apply_event(
        record,
        ControlPlaneEvent(
            event=SourceEvent.SESSION_START,
            run_id="run",
    ...
    stopped = gate.apply_event(
        updated,
        ControlPlaneEvent(
            event=SourceEvent.SESSION_END, run_id="run", session_id="new-session"
        ),
    )
    assert stopped.gate_state == GateState.FINISHED_PENDING_GATE
    ignored = gate.apply_event(
        stopped,
        ControlPlaneEvent(
            event=SourceEvent.SESSION_IDLE, run_id="run", session_id="new-session"
        ),
    )
...
def test_apply_event_sets_idle_question_and_stop_states() -> None:
    gate = WakeGate(_config(), _StaticTracker(), _StaticTelemetry())
    record = _record()
    idle = gate.apply_event(
        record,
        ControlPlaneEvent(
            event=SourceEvent.SESSION_IDLE, run_id="run", session_id="session"
        ),
    )
    assert idle.gate_state == GateState.PENDING_IDLE_GATE
    assert idle.idle_seen_at is not None
    question = gate.apply_event(
        idle,
        ControlPlaneEvent(
            event=SourceEvent.ASK_USER_QUESTION, run_id="run", session_id="session"
        ),
    )
    ...
    stopped = gate.apply_event(
        question,
        ControlPlaneEvent(
            event=SourceEvent.SESSION_STOP, run_id="run", session_id="session"
        ),
    )
    assert stopped.gate_state == GateState.FINISHED_PENDING_GATE
...
def test_reconcile_promotes_running_record_when_processes_are_gone() -> None:
    ...
    busy_gate = WakeGate(
        _config(),
        _StaticTracker(ProcessSnapshot(process_alive=True)),
        _StaticTelemetry(),
    )
...
def test_evaluate_waits_for_process_exit_and_then_emits_callbacks() -> None:
    busy = WakeGate(
        _config(),
        _StaticTracker(ProcessSnapshot(descendants_alive=True)),
        _StaticTelemetry(),
    )
    ...
    quiet_cfg = _config(
        workload_profiles={
            "training": {
                "idle_sustain_sec": 30,
                "cpu_idle_threshold_pct": 90,
                "gpu_idle_avg_threshold_pct": 90,
                "gpu_idle_peak_threshold_pct": 90,
                "vram_delta_threshold_mib": 100,
            }
        }
    )
    quiet_gate = WakeGate(
    ...
    record = _record(
        gate_state=GateState.FINISHED_PENDING_GATE,
        workload_class="training",
        baseline_vram_mib=0,
        last_event=SourceEvent.SESSION_END,
        last_event_at="seen",
    )
    for _ in range(30):
    ...
    duplicate, duplicate_callback = quiet_gate.evaluate(
        record.model_copy(update={"last_idempotency_key": callback.idempotency_key})
    )
        ...
        _StaticTelemetry(
            [
                TelemetrySample(
                    ...
                    vram_used_mib=123,
                    memory_source="uma_meminfo",
                    uma_allocatable_mib=10_000,
                    uma_pressure_mib=123,
                )
            ]
        ),
    )
    record, callback = gate.evaluate(
        _record(
...
def test_timeout_and_reaper_guards() -> None:
    gate = WakeGate(
        _config(stale_project_process_reaper_enabled=False),
        _StaticTracker(),
        _StaticTelemetry(),
    )
    assert (
        gate.reap_stale_project_processes(_record(gate_state=GateState.RUNNING)) == []
    )
    assert gate.is_timed_out(_record(gate_state=GateState.RUNNING)) is False
    old = "2026-01-01T00:00:00+00:00"
    assert (
        gate.is_timed_out(
            _record(gate_state=GateState.PENDING_IDLE_GATE, idle_seen_at=old)
        )
        is True
    )
    ...
    assert (
        gate.is_timed_out(
            _record(gate_state=GateState.PENDING_IDLE_GATE, idle_seen_at=naive_old)
        )
        is True
    )


def test_callback_outbox_reuses_existing_metadata_and_marks_delivered(
...
def test_callback_outbox_failure_and_replay_limits(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        ...
        "deliver_payload",
        lambda payload, **kwargs: callback_outbox.DeliveryResult(
            ok=False, status_code=500, detail="boom"
        ),
    )
    result = callback_outbox.deliver_pending_file(
        first,
...
def test_deliver_payload_rejects_file_scheme_before_urlopen(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    called = False
...
def test_process_helpers_cover_benign_and_same_process(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert (
        tracker._project_dir(_record(project_dir=""))
    ...
    assert (
        tracker._same_process(
            SimpleNamespace(create_time=lambda: 1.005), SimpleNamespace(create_time=1.0)
        )
        is True
    )
    assert (
        tracker._same_process(
            SimpleNamespace(create_time=lambda: 2.0), SimpleNamespace(create_time=1.0)
        )
        is False
    )
...
def test_reaper_sends_sigkill_when_term_does_not_exit(
    ...
    monkeypatch.setattr(
        ...
        lambda _pid: SimpleNamespace(
            create_time=lambda: 1.0, is_running=lambda: True, status=lambda: "running"
        ),
    )
    reaped = tracker.reap_stale_project_processes(
        _record(), stale_after_sec=0, command_markers=["python"], term_grace_sec=1
    )
...
def test_meminfo_and_uma_memory_paths(tmp_path: Path) -> None:
    ...
    huge = telemetry_mod._uma_memory_from_meminfo(
        {
            "MemTotal": 4096000,
            "HugePages_Total": 4,
            "HugePages_Free": 2,
...
def test_telemetry_collector_without_optional_backends(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_telemetry_collector_uses_nvml_dedicated_memory(
    ...
    monkeypatch.setattr(
        telemetry_mod, "psutil", SimpleNamespace(cpu_percent=cpu_percent)
    )
    monkeypatch.setattr(
        telemetry_mod,
        "_read_meminfo",
