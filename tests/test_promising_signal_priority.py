...
def test_promising_signal_bucket_matches_export_ranking_contract():
    ...
    assert promising_signal_bucket(_row(compute_scale_blocked=True)) == "compute_scale_blocked"
    assert promising_signal_bucket(_row(hypothesis_status="unsupported", evidence_strength="weak")) == "likely_stale_low_value_archive"
...
def test_ranked_followup_readiness_requires_bounded_evidence_and_excludes_stale():
    ...
    sparse = ranked_followup_readiness(_row(followup_required_evidence=["one metric only"]))
    assert sparse["ready"] is False
    assert "required_evidence" in sparse["reason"]

    scale_only = ranked_followup_readiness(_row(
        compute_scale_blocked=True,
        followup_required_evidence=["datacenter 70B training run", "multi-gpu replication"],
...
def test_ranked_followup_priority_orders_top_and_bounded_scale_before_regular_followups():
    ...
    scale = _row(project_id="scale", compute_scale_blocked=True, evidence_strength="moderate", updated_at="2026-05-17T00:00:00+00:00")
    regular = _row(project_id="regular", evidence_strength="weak", hypothesis_status="mixed", source_url="", updated_at="2026-05-19T00:00:00+00:00")
...
def test_local_store_next_followup_candidate_uses_ranked_priority(monkeypatch, tmp_path):
    store = ControlPlaneStore(tmp_path / "control.sqlite3")
    top = _row(project_id="top", updated_at="2026-05-18T00:00:00+00:00")
    scale = _row(project_id="scale", compute_scale_blocked=True, evidence_strength="moderate", updated_at="2026-05-17T00:00:00+00:00")
    regular_newer = _row(project_id="regular", evidence_strength="weak", hypothesis_status="mixed", source_url="", updated_at="2026-05-19T00:00:00+00:00")
    stale = _row(project_id="stale", hypothesis_status="unsupported", evidence_strength="weak", updated_at="2026-05-20T00:00:00+00:00")

    monkeypatch.setattr(store, "operator_queue_rows_sql", lambda: [regular_newer, stale, scale, top])
...
def test_local_store_explicit_project_can_select_stale_archive(monkeypatch, tmp_path):
    store = ControlPlaneStore(tmp_path / "control.sqlite3")
    stale = _row(project_id="stale", hypothesis_status="unsupported", evidence_strength="weak")
