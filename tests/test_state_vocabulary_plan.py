from scripts.generate_state_vocabulary_plan import (
    DOMAIN_TARGETS,
    cleanup_action,
    final_state_for,
    iter_mapping_rows,
...
def test_state_vocabulary_plan_uses_only_declared_final_states_and_actions() -> None:
    allowed_states = {
        domain: set(config["states"]) for domain, config in DOMAIN_TARGETS.items()
    }
...
def test_state_vocabulary_plan_locks_high_risk_boundaries() -> None:
    assert (
        final_state_for("project_decisions.decision_gate_state", "positive")
        == "paper_positive"
    )
    assert (
        final_state_for("project_decisions.decision_gate_state", "negative")
        == "done_no_paper"
    )
    assert (
        final_state_for("project_decisions.decision_gate_state", "needs_review")
        == "done_no_paper"
    )
...
def test_rendered_state_vocabulary_plan_contains_migration_safe_table() -> None:
    ...
    assert (
        "| `retire` | Historical/import/provenance value accepted for audit only."
        in text
    )
    assert (
        "`write_needed`" not in text
    )  # this is a paper-pipeline counter, not a raw state vocabulary item
