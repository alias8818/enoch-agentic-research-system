        ...
        [
            {
                ...
                "janitor_action": {
                    "dispatch_priority": {"dispatch_priority_score": 65.0}
                },
            }
        ]
    )
...
def test_llm_review_maps_non_admit_decisions_to_candidate_statuses():
    assert (
        research_facility_llm_review._candidate_status_for_decision(
    ...
    assert (
        research_facility_llm_review._candidate_status_for_decision(
            {"decision": "rewrite_contract", "confidence": "high"}
        )
        == "rewrite_needed"
    )
    assert (
        research_facility_llm_review._candidate_status_for_decision(
            {"decision": "keep_for_later", "confidence": "medium"}
        )
        == "deferred"
    )
...
def test_llm_review_low_confidence_admit_is_deferred_not_admitted():
    assert (
        research_facility_llm_review._candidate_status_for_decision(
            {"decision": "admit", "confidence": "low"}
        )
        == "deferred"
    )
    assert (
        research_facility_llm_review._candidate_status_for_decision(
            {"decision": "admit", "confidence": "medium"}
        )
        == "admitted"
    )
...
def test_llm_review_non_admit_conflicts_on_reused_admission_key_with_different_identity() -> (
    None
):
    decision = {
        "candidate_id": "candidate-1",
    ...
    class Cursor:
        ...
        def execute(self, sql, params=()):
            ...
            if normalized.startswith("select admission_id"):
                assert params == (
                    "research-janitor-llm-admission:candidate-1:deferred",
                )
                self._fetchone = {
                    "admission_id": 9,
                    "candidate_id": "candidate-1",
...
def test_llm_review_non_admit_does_not_record_admission_when_status_update_skips(
    monkeypatch,
) -> None:
    decision = {
        "candidate_id": "candidate-1",
    ...
    monkeypatch.setattr(
        research_facility_llm_review, "_insert_research_admission", fake_insert
    )
