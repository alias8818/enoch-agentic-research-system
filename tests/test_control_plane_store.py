...
class ControlPlaneStoreTests(unittest.TestCase):
    ...
    def test_next_dispatch_candidate_normalizes_status_and_review_flag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                store.next_dispatch_candidate()["project_id"], "queued-string-false"
            )
    ...
    def test_next_followup_candidate_requires_concrete_evidence_list(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base = {
                ...
                "scale_limits": "local CPU validation only",
                "useful_signal_summary": "bounded follow-up signal with evidence",
            ...
            self.assertEqual(
                store.launch_followup_candidate(dry_run=True)["action"], "noop"
            )
    ...
    def test_import_snapshot_defaults_missing_origin_status_to_unknown(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                store.project_row("missing-status-queue")["origin_idea_status"],
                "unknown",
            )
            self.assertEqual(
                store.project_row("missing-status-paper-project")["origin_idea_status"],
                "unknown",
            )

    def test_import_ideas_preserves_existing_origin_status_when_replayed_with_blank_status(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            self.assertEqual(
                store.project_row("origin-status-project")["origin_idea_status"],
                "testing",
            )
    ...
    def test_native_intake_routes_agent_integrity_ledger_to_cpu_worker_target(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")

            _inserted, _created, _updated, _skipped, candidates, _skipped_rows = (
                store.ingest_ideas(
                    IdeaIntakeRequest(
                        source="unit",
            ...
            self.assertEqual(
                candidates[0]["routing_reason"], "workload_class:agent_harness"
            )
    ...
    def test_import_snapshot_rejects_conflicting_duplicate_queue_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(
                ValueError, "conflicting queue project identity"
            ):
    ...
    def test_import_snapshot_rejects_conflicting_rows_inside_snapshot_payload(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_import_snapshot_row_failure_does_not_consume_idempotency_key(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(
                store, "_connect", side_effect=failing_connect
            ):
    ...
    def test_pause_append_failure_does_not_mutate_control_flags(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(
                store, "_append_event_in_conn", side_effect=fail_append_event
            ):
                with self.assertRaises(RuntimeError):
                    store.pause(
                        reason="maintenance", paused_by="test", maintenance_mode=True
                    )
    ...
    def test_import_snapshot_is_idempotent_and_selects_candidate_after_resume(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            store.resume(resumed_by="test", maintenance_mode=False)
            before_events = len(store.recent_events(100))
            action, candidate, event_id, _ = store.dispatch_next_dry_run(
                requested_by="test"
            )
    ...
    def test_import_snapshot_idempotency_replay_does_not_rewrite_runtime_state(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            self.assertEqual(
                before["last_result_summary"], "operator pause after import"
            )
            ...
            self.assertEqual(
                after["last_result_summary"], before["last_result_summary"]
            )
    ...
    def test_notion_intake_replay_does_not_rewrite_queue_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inserted_again, created_again, updated_again, *_ = (
            ...
            with self.assertRaises(IdempotencyConflict):
                store.ingest_notion_ideas(
                    payload.model_copy(update={"notion_rows": []})
                )
    ...
    def test_native_intake_row_failure_does_not_consume_idempotency_key(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            class FailingConnection:
                ...
                def execute(self, sql, params=()):
                    ...
                    if fail_project_insert and "INSERT INTO projects" in str(sql):
                        fail_project_insert = False
                        raise RuntimeError(
                            "simulated native intake project write failure"
                        )
                    return self.conn.execute(sql, params)
            ...
            with unittest.mock.patch.object(
                store,
                "_connect",
                side_effect=lambda: FailingConnection(original_connect()),
            ):
                with self.assertRaises(RuntimeError):
    ...
    def test_legacy_notion_reingest_preserves_runtime_project_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(after["project_dir"], before["project_dir"])
            self.assertEqual(
                after["notion_page_url"],
                "https://www.notion.so/Preserve-Project-Dir-00000000000040008000000000000005",
            )
    ...
    def test_notion_intake_can_override_existing_dispatch_metadata_explicitly(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_import_snapshot_preserves_active_runtime_even_when_current_run_id_missing(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_active_queue_with_blank_current_run_does_not_attach_stale_project_paper(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_dispatch_claim_prevents_second_dispatch_before_worker_call(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first = store.claim_dispatch_candidate(
                project_id="idea-claim", run_id="run-claim-1", requested_by="pump-a"
            )
            second = store.claim_dispatch_candidate(
                project_id="idea-claim", run_id="run-claim-2", requested_by="pump-b"
            )
            ...
            self.assertEqual(
                store.next_dispatch_candidate()["project_id"], "idea-claim"
            )
    ...
    def test_next_dispatch_candidate_skips_only_active_worker_target(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            action, dry_candidate, _event_id, reason = store.dispatch_next_dry_run(
                requested_by="test"
            )
            self.assertEqual(action, "dry_run_dispatch")
    ...
    def test_dispatch_claim_allows_different_worker_target_but_blocks_same_target(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_worker_callback_normalizes_event_type_and_gate_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                row["next_action_hint"], "draft_paper_or_select_next_project"
            )
            ...
            event = store.event_rows(
                limit=1, entity_type="run", entity_id="run-normalized-callback"
            )[0]
            self.assertEqual(event["event_type"], "worker_callback.wake_ready")

    def test_worker_callback_creates_missing_run_row_when_queue_claim_exists(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_dispatch_started_idempotent_replay_does_not_regress_completed_queue(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_release_dispatch_claim_does_not_emit_event_when_no_claim_released(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            events = store.event_rows(
                limit=10, entity_type="project", entity_id="idea-release-stale"
            )
            self.assertEqual([event["event_type"] for event in events], [])

    def test_worker_callback_missing_run_id_does_not_mutate_active_project(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_worker_callback_missing_run_id_does_not_mutate_active_project_with_empty_current_run(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_worker_callback_missing_run_id_does_not_complete_queued_project(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_worker_callback_foreign_run_does_not_complete_unclaimed_project(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_stale_worker_callback_replay_stays_idempotent_after_current_run_completes(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_mark_queue_item_paused_append_failure_does_not_mutate_queue_state(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_dispatch_claim_idempotent_replay_does_not_reclaim_released_queue(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            claimed = store.claim_dispatch_candidate(
                project_id=project_id, run_id=run_id, requested_by="test"
            )
            ...
            self.assertEqual(after["next_action_hint"], before["next_action_hint"])
            events = store.event_rows(
                limit=10, entity_type="project", entity_id=project_id
            )
            self.assertEqual(
    ...
    def test_mark_dispatch_started_append_failure_does_not_mutate_runtime_state(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            self.assertEqual(
                after_queue["current_run_id"], before_queue["current_run_id"]
            )
            self.assertEqual(
                after_queue["current_session_id"], before_queue["current_session_id"]
            )
            self.assertEqual(
                after_queue["last_run_state"], before_queue["last_run_state"]
            )
    ...
    def test_worker_callback_append_failure_does_not_mutate_runtime_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                after_queue["next_action_hint"], before_queue["next_action_hint"]
            )
    ...
    def test_worker_callback_without_idempotency_key_replays_by_run_event_and_session(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                len(
                    store.event_rows(
                        limit=10, entity_type="run", entity_id="run-missing-idempotency"
                    )
                ),
                1,
            )
            ...
            third_event_id, third_inserted, _third_row = store.record_worker_callback(
                changed_callback
            )
    ...
    def test_worker_callback_without_identifiers_dedupes_by_payload(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first_event_id, first_inserted, _first_row = store.record_worker_callback(
                callback
            )
            second_event_id, second_inserted, _second_row = (
                store.record_worker_callback(callback)
            )
            ...
            self.assertFalse(second_inserted)
            self.assertEqual(
                len(store.event_rows(limit=10, entity_type="run", entity_id="unknown")),
                1,
            )
    ...
    def test_upsert_paper_preserves_existing_review_row(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inserted, created, _updated, _skipped, errors = (
    ...
    def test_upsert_paper_rejects_conflicting_paper_identity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            paper_id = "paper-upsert-conflict:run-1:arxiv_draft"
            store.import_snapshot(
                ImportSnapshotRequest(
                    idempotency_key="paper-upsert-conflict-projects",
                    queue_rows=[
                        {
                            "project_id": "project-a",
            ...
            store.upsert_paper(
                PaperRecord(
                    ...
                    run_id="run-1",
                    paper_status=PaperStatus.PUBLICATION_DRAFT,
                    draft_markdown_path="papers/run-1/paper.md",
                )
            )
    ...
    def test_dashboard_observations_store_latest_by_source_and_scope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                store.latest_dashboard_observations()[
                    "worker_preflight"
                ].observation_id,
                second.observation_id,
            )

    def test_dashboard_observations_latest_prefers_observed_at_over_insert_order(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
    ...
    def test_imports_raw_wake_gate_snapshots_and_builds_notion_projections(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            queue_projection = {
                row["project_id"]: row for row in store.queue_notion_projection()
            }
            self.assertEqual(
                queue_projection["idea-active"]["queue_status"], "awaiting_wake"
            )
    ...
    def test_paper_review_backfill_is_idempotent_and_ranks_publication_drafts(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            store.import_snapshot(
                ImportSnapshotRequest(
                    ...
                    queue_rows=[
                        {
                            "project_id": "pub",
                            "project_name": "Publication Project",
                            "status": "completed",
                        },
                        {
                            "project_id": "draft",
            ...
            audit_path.write_text(
                '{"papers":[{"paper_id":"pub:run-1:arxiv_draft","ready":true},{"paper_id":"draft:run-1:arxiv_draft","ready":true}]}',
                encoding="utf-8",
            )
            dry_inserted, dry_created, _, dry_skipped, dry_errors = (
                store.backfill_paper_reviews(
                    PaperReviewBackfillRequest(
                        idempotency_key="review-backfill-1",
            ...
            (
                ...
                PaperReviewBackfillRequest(
                    idempotency_key="review-backfill-2",
                    source_audit_path=str(audit_path),
                    dry_run=False,
                )
            )
            ...
            self.assertEqual(
                (created_again, updated_again, skipped_again, errors_again),
                (0, 0, 2, []),
            )
    ...
    def test_paper_review_backfill_upserts_stale_ranking_fields(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="review-backfill-stale-1", dry_run=False
                )
            )
            ...
            inserted, created, updated, skipped, errors = store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="review-backfill-stale-2",
                    source_audit_path=str(audit_path),
                    dry_run=False,
                )
            )
    ...
    def test_paper_review_backfill_records_missing_paths_as_errors(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inserted, created, updated, skipped, errors = store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="review-backfill-missing", dry_run=False
                )
            )
            self.assertTrue(inserted)
            self.assertEqual((created, updated, skipped), (1, 0, 0))
    ...
    def test_paper_review_claim_checklist_status_and_approval_events(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="review-mutation-backfill",
                    source_audit_path=str(audit_path),
                    dry_run=False,
                )
            )
            ...
            event_id, inserted, item = store.claim_paper_review(
                paper_id,
                PaperReviewClaimRequest(
                    idempotency_key="claim-1", requested_by="alice", reviewer="alice"
                ),
            )
            self.assertTrue(inserted)
            self.assertEqual(item["review_status"], "claimed")
            self.assertEqual(item["reviewer"], "alice")
            event_id_again, inserted_again, _ = store.claim_paper_review(
                paper_id,
                PaperReviewClaimRequest(
                    idempotency_key="claim-1", requested_by="alice", reviewer="alice"
                ),
            ...
            with self.assertRaises(ValueError):
                store.update_paper_review_checklist(
                    paper_id,
                    "artifact_readability",
                    PaperReviewChecklistUpdateRequest(
                        idempotency_key="checklist-fail-no-note",
                        requested_by="alice",
                        status="fail",
                    ),
                )
            with self.assertRaises(ValueError):
                store.approve_paper_review_finalization(
                    paper_id,
                    PaperReviewApproveFinalizationRequest(
                        idempotency_key="approve-too-soon", requested_by="alice"
                    ),
                )
            ...
            for item_id in required_items:
                store.update_paper_review_checklist(
                    ...
                    item_id,
                    PaperReviewChecklistUpdateRequest(
                        idempotency_key=f"checklist-pass-{item_id}",
                        requested_by="alice",
                        status="pass",
                    ),
                )
            ...
            with self.assertRaises(ValueError):
                store.approve_paper_review_finalization(
                    paper_id,
                    PaperReviewApproveFinalizationRequest(
                        idempotency_key="approve-1", requested_by="alice", note="ready"
                    ),
                )
            self.assertEqual(
                store.paper_row(paper_id)["paper_status"], "publication_draft"
            )
            events = store.event_rows(entity_id=paper_id, limit=50)
            self.assertTrue(
                any(event["event_type"] == "paper_review.claimed" for event in events)
            )
            self.assertTrue(
                any(
                    event["event_type"] == "paper_review.checklist_updated"
                    for event in events
                )
            )
            self.assertFalse(
                any(
                    event["event_type"] == "paper_review.approved_for_finalization"
                    for event in events
                )
            )
    ...
    def test_prepare_finalization_package_dry_run_commit_and_retry(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="package-backfill",
                    source_audit_path=str(audit_path),
                    dry_run=False,
                )
            )
            _event_id, _inserted, _item = store.claim_paper_review(
                paper_id,
                PaperReviewClaimRequest(
            ...
            for item_id in [
                ...
                store.update_paper_review_checklist(
                    ...
                    item_id,
                    PaperReviewChecklistUpdateRequest(
                        idempotency_key=f"package-check-{item_id}",
                        requested_by="alice",
                        status="pass",
                    ),
                )
            ...
            self.assertTrue(
                all(artifact["readable"] for artifact in manifest["artifacts"])
            )
            ...
            with unittest.mock.patch(
                "enoch_control_plane.control_plane.store._atomic_write_text",
            ...
            self.assertEqual(
                existing_manifest.read_text(encoding="utf-8"), "previous manifest"
            )
            existing_manifest.unlink()

            event_id, inserted, finalized, package_path, manifest = (
                store.prepare_paper_review_finalization_package(
                    paper_id,
                    PaperReviewPrepareFinalizationRequest(
                        idempotency_key="package-commit",
                        requested_by="alice",
                        target_label="first-paper",
                        dry_run=False,
                    ),
    ...
    def test_prepare_finalization_package_event_failure_restores_manifest_and_review_state(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="package-event-fail-backfill", dry_run=False
                )
            )
            store.claim_paper_review(
                paper_id,
                PaperReviewClaimRequest(
                    idempotency_key="package-event-fail-claim",
                    requested_by="alice",
                    reviewer="alice",
                ),
            )
            package_path = store._finalization_manifest_path(
                paper_id, "package-event-fails"
            )
            ...
            with unittest.mock.patch.object(
                ...
                with self.assertRaises(RuntimeError):
                    store.prepare_paper_review_finalization_package(
                        paper_id,
                        PaperReviewPrepareFinalizationRequest(
                            idempotency_key="package-event-fails",
                            requested_by="alice",
                            target_label="first-paper",
                            dry_run=False,
                        ),
            ...
            self.assertEqual(
                package_path.read_text(encoding="utf-8"), "previous manifest"
            )
            ...
            self.assertFalse(
                any(
                    event["event_type"] == "paper_review.finalization_package_prepared"
                    for event in store.event_rows(entity_id=paper_id, limit=50)
                )
            )
    ...
    def test_existing_file_snapshot_fails_closed_on_access_errors(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch.object(Path, "exists", blocked_exists):
                with self.assertRaisesRegex(
                    ValueError,
                    "could not inspect existing finalization package manifest",
                ):
                    _existing_file_snapshot(
                        manifest_path, label="finalization package manifest"
                    )
            ...
            with unittest.mock.patch.object(Path, "read_bytes", blocked_read_bytes):
                with self.assertRaisesRegex(
                    ValueError, "could not read existing finalization package manifest"
                ):
                    _existing_file_snapshot(
                        manifest_path, label="finalization package manifest"
                    )
            ...
            with self.assertRaisesRegex(
                ValueError, "existing finalization package manifest is not a file"
            ):
                _existing_file_snapshot(
                    directory_path, label="finalization package manifest"
                )

    def test_prepare_finalization_package_rejects_empty_evidence_artifacts(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            for rel in artifact_paths.values():
                (project_dir / rel).write_text(
                    "{}" if rel.endswith(".json") else "content", encoding="utf-8"
                )
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="empty-evidence-review-backfill", dry_run=False
                )
            )
            ...
            with self.assertRaisesRegex(ValueError, "semantic evidence gate"):
                store.prepare_paper_review_finalization_package(
                    paper_id,
                    PaperReviewPrepareFinalizationRequest(
                        idempotency_key="empty-evidence-finalize",
                        requested_by="test",
                        dry_run=False,
                    ),
                    require_approval=False,
    ...
    def test_prepare_finalization_package_rechecks_existing_finalized_artifacts(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            package_path.write_text(
                '{"semantic_evidence_gate":{"ok":false}}\n', encoding="utf-8"
            )
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="stale-finalized-review-backfill", dry_run=False
                )
            )
            ...
            with self.assertRaisesRegex(ValueError, "semantic evidence gate"):
                store.prepare_paper_review_finalization_package(
                    paper_id,
                    PaperReviewPrepareFinalizationRequest(
                        idempotency_key="stale-finalized-recheck",
                        requested_by="test",
                        dry_run=False,
                    ),
                    require_approval=False,
    ...
    def test_paper_review_status_validation_blocks_invalid_transitions(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="review-status-backfill", dry_run=False
                )
            )
            with self.assertRaises(ValueError):
                store.update_paper_review_status(
                    paper_id,
                    PaperReviewStatusUpdateRequest(
            ...
            with self.assertRaises(ValueError):
                store.claim_paper_review(
                    paper_id,
                    PaperReviewClaimRequest(
                        idempotency_key="claim-blocked",
                        requested_by="alice",
                        reviewer="alice",
                    ),
                )
            _event_id, _inserted, claimed = store.claim_paper_review(
                paper_id,
                PaperReviewClaimRequest(
                    idempotency_key="claim-cleared",
                    requested_by="alice",
                    reviewer="alice",
                    clear_blocker=True,
                ),
    ...
    def test_claim_paper_review_update_failure_does_not_consume_idempotency_key(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="claim-atomic-backfill", dry_run=False
                )
            )
            original_connect = store._connect
            ...
            class FailingConnection:
                ...
                def execute(self, sql, params=()):
                    ...
                    if fail_review_update and "UPDATE paper_review_items" in str(sql):
                        ...
                        raise RuntimeError(
                            "simulated paper review claim update failure"
                        )
                    return self.conn.execute(sql, params)
            ...
            with unittest.mock.patch.object(
                ...
                with self.assertRaises(RuntimeError):
                    store.claim_paper_review(
                        paper_id,
                        PaperReviewClaimRequest(
                            idempotency_key="claim-atomic-key",
                            requested_by="alice",
                            reviewer="alice",
                        ),
                    )
            ...
            self.assertTrue(
                any(
                    ...
                    and event["event_type"] == "paper_review.claimed"
                    for event in store.event_rows(entity_id=paper_id, limit=50)
                )
            )

    def test_backfill_paper_reviews_row_failure_does_not_consume_idempotency_key(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.sqlite3")
            ...
            class FailingConnection:
                ...
                def execute(self, sql, params=()):
                    ...
                    if fail_review_insert and "INSERT INTO paper_review_items" in str(
                        sql
                    ):
                        fail_review_insert = False
                        raise RuntimeError(
                            "simulated paper review backfill insert failure"
                        )
                    return self.conn.execute(sql, params)

            request = PaperReviewBackfillRequest(
                idempotency_key="backfill-atomic-key", dry_run=False
            )
            with unittest.mock.patch.object(
                store,
                "_connect",
            ...
            self.assertEqual(errors, [])
            self.assertEqual(
                store.paper_review_row(paper_id)["review_status"], "queued"
            )
    ...
    def test_paper_finalization_rejects_artifacts_outside_project_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.backfill_paper_reviews(
                PaperReviewBackfillRequest(
                    idempotency_key="escape-review-backfill", dry_run=False
                )
            )
            ...
            with self.assertRaisesRegex(ValueError, "readable artifacts"):
                store.prepare_paper_review_finalization_package(
                    paper_id,
                    PaperReviewPrepareFinalizationRequest(
                        idempotency_key="escape-finalize",
                        requested_by="test",
                        dry_run=False,
                    ),
                    require_approval=False,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload = NotionIntakeRequest(
                ...
                notion_rows=[
                    ...
                    {
                        "id": "discard-me",
                        "property_idea": "Discarded",
                        "property_status": "discarded",
                    },
            ...
            inserted, created, updated, skipped, candidates, skipped_rows = (
                store.ingest_notion_ideas(committed)
            )
            self.assertTrue(inserted)
            ...
            self.assertEqual(
                rows[0]["notion_page_id"], "00000000-0000-4000-8000-000000000001"
            )
            updates = store.notion_execution_update_projection()
            self.assertEqual(
                updates[0]["page_id"], "00000000-0000-4000-8000-000000000001"
            )
            self.assertEqual(updates[0]["properties"]["Execution State"], "queued")
            self.assertEqual(
                updates[0]["properties"]["Next Action"], "controller_review"
            )
...
def test_control_plane_store_closes_sqlite_connections_after_context(
    monkeypatch,
) -> None:
    closed = 0
    ...
    monkeypatch.setattr(
        "enoch_control_plane.control_plane.store.sqlite3.connect",
