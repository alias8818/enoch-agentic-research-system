...
class ControlPlaneRouterTests(unittest.TestCase):
    ...
    def test_control_dashboard_legacy_redirects_while_v2_shell_served(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("CONTROL_DASHBOARD_HTML", Path("enoch_control_plane/control_plane/router.py").read_text(encoding="utf-8"))
    ...
    def test_research_facility_backpressure_event_idempotency_conflict_still_returns_ok(self) -> None:
        ...
        self.assertEqual(body["reason"], "active worker lane already exists and no promotable candidate targets an idle lane")
    ...
    def test_research_facility_run_cycle_dispatches_idle_gb10_when_cpu_lane_active(self) -> None:
        class FakeSupabaseStore:
            def __init__(self) -> None:
                self.events = []
                self.observations = []
            ...
            def status_counts(self) -> dict[str, int]:
                return {"blocked": 0, "queued": len([r for r in self.queue.values() if r["status"] == "queued"]), "active": 1}
            ...
            def research_facility_workbench_projection(self, *, limit: int = 100) -> list[dict[str, str]]:
                return [
                    {"candidate_id": "cpu-high-score", "title": "CPU High", "admission_decision": "admitted", "admitted_idea_id": "", "total_score": "99.00", "category": "agent-reliability", "machine_target": "cpu-proxmox-1"},
                    {"candidate_id": "gb10-open-lane", "title": "GB10 Open", "admission_decision": "admitted", "admitted_idea_id": "", "total_score": "80.00", "category": "spec-decoding", "machine_target": "gb10"},
                ]
        ...
        self.assertEqual(body["dispatches"][0]["candidate"]["project_id"], "gb10-open-lane")
    ...
    def test_dashboard_status_treats_worker_settling_after_vm_completion_as_backpressure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(item["source"] == "worker_settling" for item in alert["findings"]))
    ...
    def test_dashboard_status_does_not_treat_future_worker_settling_without_vm_match_as_backpressure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            client = _client_with_config(_config(tmp).model_copy(update={"live_dispatch_enabled": True}))
            headers = {"Authorization": f"Bearer {TOKEN}"}
            client.post("/control/resume", headers=headers, json={"resumed_by": "test", "maintenance_mode": False})
            ...
            self.assertNotIn("GB10 worker settling recent run", status["dispatch_blockers"])
            self.assertIn("GB10/VM active-lane conflict", status["dispatch_blockers"])
            self.assertTrue(any(item["severity"] == "critical" for item in status["conflicts"]))
