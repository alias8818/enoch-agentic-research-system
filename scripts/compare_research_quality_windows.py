...
from enoch_control_plane.research_quality.datasets import CandidateRow, pairwise_similarity  # noqa: E402
from scripts.build_research_quality_evalset import RawDecision, build_eval_cases  # noqa: E402
from scripts.dspy_research_quality import _candidate_from_mapping, _decision_from_mapping  # noqa: E402
...
def summarize_window(*, candidates: list[CandidateRow], decisions: list[RawDecision], max_followup_depth: int) -> dict[str, Any]:
    cases = build_eval_cases(candidates, decisions, max_followup_depth=max_followup_depth)
    ...
    return {
        ...
        "admitted_rate": round(status_counts.get("admitted", 0) / len(candidates), 3) if candidates else 0.0,
        "moonshot_count": len(moonshots),
        "moonshot_avg_score": round(sum(moonshot_scores) / len(moonshot_scores), 3) if moonshot_scores else 0.0,
...
def compare_windows(pre: dict[str, Any], post: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "max_depth_followup_delta": round(
            ...
            - float(pre.get("eval_case_counts", {}).get("max_depth_followup_ending", 0)),
            3,
        ),
...
def _fetch_window(database_url: str, *, cutoff: str, side: str, limit: int) -> tuple[list[CandidateRow], list[RawDecision], dict[str, Any]]:
    import psycopg
    ...
    meta = {
        ...
        "candidate_first_created_at": str(candidate_rows[0]["created_at"]) if candidate_rows else None,
        "candidate_last_created_at": str(candidate_rows[-1]["created_at"]) if candidate_rows else None,
        "decision_first_created_at": str(decision_rows[0]["created_at"]) if decision_rows else None,
        "decision_last_created_at": str(decision_rows[-1]["created_at"]) if decision_rows else None,
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--database-url", default=os.environ.get("ENOCH_CONTROL_DATABASE_URL") or os.environ.get("ENOCH_SUPABASE_DATABASE_URL") or os.environ.get("DATABASE_URL") or "")
    parser.add_argument("--cutoff", required=True, help="ISO timestamp separating pre/post windows")
    parser.add_argument("--limit", type=int, default=20, help="candidate and decision rows per window")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.database_url:
        raise SystemExit("--database-url or ENOCH_CONTROL_DATABASE_URL/ENOCH_SUPABASE_DATABASE_URL/DATABASE_URL is required")
    pre_candidates, pre_decisions, pre_meta = _fetch_window(args.database_url, cutoff=args.cutoff, side="pre", limit=args.limit)
    post_candidates, post_decisions, post_meta = _fetch_window(args.database_url, cutoff=args.cutoff, side="post", limit=args.limit)
    pre = summarize_window(candidates=pre_candidates, decisions=pre_decisions, max_followup_depth=args.max_followup_depth)
    post = summarize_window(candidates=post_candidates, decisions=post_decisions, max_followup_depth=args.max_followup_depth)
    report = {
        "ok": True,
