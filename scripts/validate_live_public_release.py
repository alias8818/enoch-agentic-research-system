...
def reject_stale(name: str, text: str, failures: list[str]) -> None:
    lowered = text.lower()
    for phrase in STALE:
        require(phrase.lower() not in lowered, f"{name} contains stale/unscoped phrase: {phrase}", failures)
...
def main(argv: list[str] | None = None) -> int:
    ...
    expected_count = int(args.expected_count if args.expected_count is not None else manifest.get("artifact_count"))
    expected_strict_pass = int(args.expected_strict_pass if args.expected_strict_pass is not None else manifest.get("strict_claim_evidence_pass_count"))
    ...
    require(f"{expected_strict_pass}/{expected_count}" in launch, f"launch missing strict audit {expected_strict_pass}/{expected_count} status", failures)
    require("not peer-reviewed" in launch, "launch missing not-peer-reviewed caveat", failures)
    require(f'name="enoch-corpus-count" content="{expected_count}"' in launch, "launch missing corpus-count meta", failures)
    require('name="enoch-build-sha"' in launch and '__GITHUB_SHA__' not in launch, "launch missing injected build SHA", failures)

    require(str(expected_count) in profile, f"profile missing {expected_count} count", failures)
    require(f"{expected_count}/{expected_count}" in profile, f"profile missing {expected_count}/{expected_count} pass count", failures)
    require(f"{expected_strict_pass}/{expected_count}" in profile, f"profile missing strict audit {expected_strict_pass}/{expected_count} status", failures)
    ...
    require(manifest.get("artifact_count") == expected_count, f"manifest artifact_count != {expected_count}", failures)
    require(manifest.get("packaging_provenance_pass_count") == expected_count, f"manifest pass_count != {expected_count}", failures)
    require(manifest.get("gate_name") == args.expected_gate, f"manifest gate_name != {args.expected_gate}", failures)
    require(manifest.get("strict_claim_evidence_pass_count") == expected_strict_pass, f"manifest strict audit pass count should be {expected_strict_pass}", failures)
    require(manifest.get("strict_claim_evidence_total_count") == expected_count, f"manifest strict audit total != {expected_count}", failures)
    ...
    require(hf_summary.get("artifact_count") == expected_count, f"HF summary artifact_count != {expected_count}", failures)
    require(hf_summary.get("strict_claim_evidence_pass_count") == expected_strict_pass, f"HF summary strict pass count != {expected_strict_pass}", failures)
    require(hf_summary.get("strict_claim_evidence_total_count") == expected_count, f"HF summary strict total != {expected_count}", failures)
    require(f"This dataset contains {expected_count} AI-generated research artifacts" in hf_readme, "HF README missing current artifact count", failures)
    require(f"Current strict claim/evidence audit status is **{expected_strict_pass} / {expected_count} passing**" in hf_readme, "HF README missing current strict audit count", failures)
