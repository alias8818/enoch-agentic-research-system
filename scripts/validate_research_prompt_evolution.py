...
def _validate_report(
    report: dict[str, Any], *, patch_path: Path, prompt_path: Path
) -> list[str]:
    failures: list[str] = []
    ...
    if report.get("optimizer_runtime_used") is not False:
        failures.append(
            "optimizer_runtime_used must be false for this deterministic gate"
        )
    artifacts = (
    ...
    if (
        artifacts.get("prompt_candidate")
        and Path(str(artifacts["prompt_candidate"])).name != prompt_path.name
    ):
        failures.append(
            "report artifacts.prompt_candidate does not match supplied prompt filename"
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="evolution_report.json from evolve_research_prompt.py",
    )
    parser.add_argument(
        "--patch",
        type=Path,
        required=True,
        help="unified diff patch from evolve_research_prompt.py",
    )
    parser.add_argument(
        "--prompt-candidate",
        type=Path,
        required=True,
        help="prompt candidate markdown artifact",
    )
    parser.add_argument(
        "--source",
        type=Path,
        default=DEFAULT_SOURCE,
        help="source file to test patch against",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    failures = _validate_report(
        report, patch_path=args.patch, prompt_path=args.prompt_candidate
    )
    if not args.patch.read_text(encoding="utf-8").strip():
        failures.append("patch is empty")
    if "Proposed Research Quality addendum" not in args.prompt_candidate.read_text(
        encoding="utf-8"
    ):
    ...
    with tempfile.TemporaryDirectory(prefix="enoch-prompt-evolution-") as temp_name:
        ...
        proc = _run(
            ["patch", "-p0"],
            cwd=tmpdir,
            stdin=_retarget_patch(args.patch.read_text(encoding="utf-8")),
        )
        if proc.returncode != 0:
            failures.append(
                f"patch did not apply cleanly: {proc.stderr.strip() or proc.stdout.strip()}"
            )
        else:
            patched_text = patched_source.read_text(encoding="utf-8")
            failures.extend(_validate_patched_source(patched_text))
            try:
                if _non_string_ast_signature(
                    original_source
                ) != _non_string_ast_signature(patched_text):
                    failures.append(
                        "patch changes executable Python structure; only prompt string changes are allowed"
                    )
