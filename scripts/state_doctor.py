...
def _load_token(*, token: str = "", token_file: str = "") -> str:
    ...
    return os.environ.get("ENOCH_CONTROL_TOKEN") or os.environ.get("ENOCH_CONTROL_PLANE_TOKEN") or ""
...
def _live_reduction_drift(live_distincts: dict[str, Any]) -> dict[str, Any]:
    ...
    for surface, rows in sorted(live_distincts.items()):
        ...
        for value, count in rows:
            ...
            if not count or disposition not in HARD_DRIFT_DISPOSITIONS | WARNING_DRIFT_DISPOSITIONS:
                continue
            item = {
                "surface": surface,
                "value": str(value),
...
def _dashboard_audit(overview: dict[str, Any]) -> dict[str, Any]:
    ...
    allowed_operator_count_keys = {lane.value for lane in OperatorLane} | AGGREGATE_OPERATOR_COUNT_KEYS
    ...
    return {
        ...
        "paper_pipeline": {key: pipeline.get(key) for key in sorted(REQUIRED_PAPER_PIPELINE_KEYS)},
...
def _control_audit(*, control_url: str, token: str) -> dict[str, Any]:
    result: dict[str, Any] = {"control_url": control_url, "overview_ok": False, "health_ok": False}
    try:
        overview = _json_request(control_url, token, "/control/api/v1/overview?active_limit=5&event_limit=5")
    ...
    try:
        health = _json_request(control_url, token, "/control/api/v1/observability/health")
...
def _corpus_audit(*, control_url: str, token: str, corpus: Path, page_size: int) -> dict[str, Any]:
...
def evaluate_report(report: dict[str, Any], *, require_corpus_synced: bool = False) -> dict[str, list[str]]:
    failures: list[str] = []
    warnings: list[str] = []

    state_contract = report.get("state_contract") or {}
    if not state_contract.get("ok"):
        failures.extend(f"state contract: {item}" for item in state_contract.get("failures") or ["failed"])

    normalization = report.get("normalization") or {}
    if normalization.get("checked") and int(normalization.get("total_rows") or 0) != 0:
        failures.append(f"normalization dry-run would update {normalization.get('total_rows')} row(s)")
    ...
    if control.get("checked"):
        if not control.get("overview_ok"):
            failures.append(f"control overview unavailable: {control.get('overview_error', 'not ok')}")
        ...
        if not control.get("health_ok"):
            warnings.append(f"control health smoke unavailable: {control.get('health_error', 'not ok')}")
    else:
        warnings.append("control-plane smoke skipped because no token/control URL was supplied")
    ...
    if corpus.get("checked"):
        ...
        if require_corpus_synced and importable:
            failures.append(f"corpus reconciliation has {importable} finalized publication draft(s) not in the public corpus")
        elif importable:
            warnings.append(f"corpus reconciliation has {importable} finalized publication draft(s) not in the public corpus")
    elif corpus:
        warnings.append(f"corpus reconciliation skipped or failed: {corpus.get('error', 'not checked')}")
...
def _print_human(report: dict[str, Any]) -> None:
    ...
    print(f"  state contract: {'OK' if (report.get('state_contract') or {}).get('ok') else 'FAIL'}")
    normalization = report.get("normalization") or {}
    ...
    if corpus:
        if corpus.get("checked"):
            print(
                ...
                "public_corpus={public_corpus_count} importable={importable_finalized_count}".format(**corpus)
            )
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the Enoch state doctor before answering operator state/count questions.")
    parser.add_argument("--database-url", default=os.environ.get("ENOCH_SUPABASE_DATABASE_URL", ""), help="Optional live Supabase/Postgres URL. Omitted from output.")
    parser.add_argument("--control-url", default=os.environ.get("ENOCH_CONTROL_URL") or os.environ.get("ENOCH_CONTROL_PLANE_URL") or "http://127.0.0.1:8787")
    parser.add_argument("--token", default="", help="Control-plane bearer token. Prefer --token-file/env; value is never printed.")
    parser.add_argument("--token-file", default="", help="File containing the control-plane bearer token.")
    parser.add_argument("--corpus", type=Path, default=None, help="Optional public corpus checkout for publication/corpus reconciliation.")
    parser.add_argument("--page-size", type=int, default=200)
    parser.add_argument("--warn-only-corpus", action="store_true", help="Downgrade corpus/public count drift to a warning. Default fails when --corpus is checked and importable finalized drafts remain.")
