...
try:
    from sync_corpus_import_ledger import DEFAULT_CORPUS_REPO, load_public_records, sql_literal
except ModuleNotFoundError:  # pragma: no cover - import path used by pytest/package context
    from scripts.sync_corpus_import_ledger import DEFAULT_CORPUS_REPO, load_public_records, sql_literal


def render_validation_sql(*, corpus: Path, corpus_repo: str = DEFAULT_CORPUS_REPO) -> str:
    records = load_public_records(corpus)
    if not records:
        raise ValueError("no public corpus records with source_record_fingerprint found")
    values = ",\n    ".join(
        "(" + ",".join(sql_literal(value) for value in (record.source_record_fingerprint, record.artifact_slug, record.public_artifact_id)) + ")"
...
def _run_supabase_query(sql_path: Path, *, linked: bool, db_url: str) -> dict[str, Any]:
    ...
    if result.returncode != 0:
        raise SystemExit(result.stdout.strip() or f"supabase query failed with exit code {result.returncode}")
    try:
        ...
        if start >= 0 and end > start:
            try:
                ...
                pass
        raise SystemExit(f"failed to parse supabase JSON output: {exc}\n{result.stdout}") from exc
...
def validate_metrics(row: dict[str, Any]) -> list[str]:
    ...
    if imports_total != public_rows:
        failures.append(f"corpus_imports_total {imports_total} != public_index_rows {public_rows}")
    if dashboard_imported != public_rows:
        failures.append(f"dashboard_corpus_imported {dashboard_imported} != public_index_rows {public_rows}")
    if stale != 0:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--linked", action="store_true", help="Run validation against the linked Supabase project")
    parser.add_argument("--db-url", default=os.environ.get("ENOCH_SUPABASE_DATABASE_URL", ""))
    parser.add_argument("--sql-output", type=Path, help="Write validation SQL instead of running it")
    parser.add_argument("--report-json", type=Path, help="Write validation metrics JSON")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sql_output:
        ...
        print(json.dumps({"ok": True, "sql_output": str(args.sql_output)}, indent=2, sort_keys=True))
        return 0
    with tempfile.TemporaryDirectory(prefix="enoch-ledger-validate-") as tmp:
        ...
        payload = _run_supabase_query(sql_path, linked=bool(args.linked), db_url=str(args.db_url or ""))
