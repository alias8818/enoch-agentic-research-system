...
def git(
    repo: Repo, *args: str, capture: bool = False
) -> subprocess.CompletedProcess[str]:
...
def require_clean(repo: Repo) -> None:
    ...
    if status:
        raise SystemExit(
            f"{repo.key} has uncommitted changes; commit or stash before bundle push:\n{status}"
        )
...
def require_not_behind(repo: Repo) -> None:
    ...
    if behind:
        raise SystemExit(
            f"{repo.key} is behind origin/main by {behind} commit(s); pull/rebase before release"
        )
...
def remote_head(repo: Repo) -> str:
    return output(
        ["git", "ls-remote", "origin", "refs/heads/main"], cwd=repo.path
    ).split()[0]
...
def push_and_verify(repo: Repo, *, push: bool, timeout: int) -> None:
    ...
    while True:
        ...
        if time.time() >= deadline:
            raise SystemExit(
                f"{repo.key}: origin/main stayed at {remote[:12]}, expected {local[:12]}"
            )
        print(
            f"{repo.key}: waiting for origin/main {local[:12]} (currently {remote[:12]})"
        )
        time.sleep(2)


def sync_corpus_import_ledger(
    system: Repo, corpus: Repo, *, database_url: str, use_linked: bool
) -> None:
        ...
        run(
            ...
            env={
                ...
                "ENOCH_PROMISING_SIGNALS_SOURCE_CUTOFF": os.environ.get(
                    ...
                    DEFAULT_PROMISING_SIGNALS_SOURCE_CUTOFF,
                ),
            },
        )
        run(
    ...
    if not use_linked:
        raise SystemExit(
            "--sync-corpus-ledger requires --ledger-database-url/ENOCH_SUPABASE_DATABASE_URL or --ledger-use-linked"
        )
...
def run_source_lineage_check(system: Repo) -> None:
    ...
    created_after = os.environ.get(
        "ENOCH_SOURCE_LINEAGE_CREATED_AFTER", DEFAULT_SOURCE_LINEAGE_CREATED_AFTER
    )
...
def run_local_release_checks(
    ...
    with tempfile.TemporaryDirectory(prefix="enoch-release-") as tmp:
        ...
        run(
            [sys.executable, "scripts/validate_runtime_snapshot_links.py"],
            cwd=system.path,
        )
            ...
            json.dumps(
                {
                    ...
                    "packaging_provenance_pass_count": manifest.get(
                        "packaging_provenance_pass_count"
                    ),
                    "strict_claim_evidence_pass_count": manifest.get(
                        "strict_claim_evidence_pass_count"
                    ),
                    "strict_claim_evidence_total_count": manifest.get(
...
def watch_latest_workflow(repo: Repo, *, workflow: str, commit: str) -> None:
    ...
    if result.returncode != 0:
        print(
            f"warning: gh run list failed for {repo.key}; skipping workflow watch\n{result.stdout}"
        )
        return
    ...
    match = next(
        (
            row
            for row in runs
            if row.get("headSha") == commit and row.get("workflowName") == workflow
        ),
        None,
    )
    if not match:
        print(
            f"warning: no {workflow!r} run found yet for {repo.key}@{commit[:12]}; skipping watch"
        )
        return
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Race-safe push for the public Enoch multi-repo release bundle."
    )
    parser.add_argument(
        "--root",
        type=Path,
        default=Path(__file__).resolve().parents[2],
        help="Directory containing all public release repos",
    )
    parser.add_argument(
        "--push",
        action="store_true",
        help="Actually push repos. Without this, only preflight validation runs.",
    )
    parser.add_argument(
        "--watch",
        action="store_true",
        help="After pushing corpus, watch its Public release integrity workflow.",
    )
    parser.add_argument(
        "--sync-corpus-ledger",
        action="store_true",
        help="After local release checks, sync Supabase corpus_imports from the public corpus index.",
    )
    parser.add_argument(
        "--ledger-database-url",
        default=os.environ.get("ENOCH_SUPABASE_DATABASE_URL", ""),
        help="Postgres/Supabase URL for --sync-corpus-ledger; defaults to ENOCH_SUPABASE_DATABASE_URL.",
    )
    parser.add_argument(
        "--ledger-use-linked",
        action="store_true",
        help="Use `supabase db query --linked` for --sync-corpus-ledger when no database URL is available.",
    )
    parser.add_argument(
        "--allow-dirty",
        action="store_true",
        help="Allow uncommitted changes for local preflight only; never use with --push.",
    )
    parser.add_argument(
        "--remote-timeout",
        type=int,
        default=120,
        help="Seconds to wait for origin/main to match each local HEAD",
    )
    ...
    run_local_release_checks(
        system, corpus, docs, profile_site, owner_profile, personal_site, promising
    )
    ...
    print(
        "release push order:",
        " -> ".join([repo.key for repo in [*ordered_dependencies, corpus]]),
    )
    if not args.push:
