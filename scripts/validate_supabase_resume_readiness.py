...
def _request(method: str, url: str, token: str, payload: dict[str, Any] | None = None) -> HttpResult:
...
def _check_status(failures: list[str], result: HttpResult, expected: int, label: str) -> None:
    if result.status != expected:
        failures.append(f"{label} returned HTTP {result.status}, expected {expected}: {result.body}")
...
def _run_ssh_timer_check(ssh_host: str) -> dict[str, Any]:
    ...
    return {"returncode": completed.returncode, "enabled": lines[:marker], "active": lines[marker + 1 :], "raw": completed.stdout}
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not token:
        return {"ok": False, "failures": ["missing control-plane token; set ENOCH_CONTROL_PLANE_TOKEN or --token-file"]}
    ...
    legacy_projection = _request("GET", f"{base}/control/projections/notion/queue", token)
    dispatch_dry = _request("POST", f"{base}/control/dispatch-next", token, {"dry_run": True, "requested_by": "supabase-resume-readiness"})
    idea_dry = _request(
        "POST",
        f"{base}/control/intake/ideas",
        token,
        {"dry_run": True, "ideas": [{"idea_id": "resume-readiness-smoke", "title": "Resume Readiness Smoke", "idea_status": "testing"}]},
    )
    ...
    pipeline = (overview.body.get("paper_pipeline") or {}) if isinstance(overview.body, dict) else {}
    if int(pipeline.get("write_needed") or 0) != 0:
        failures.append(f"write_needed={pipeline.get('write_needed')}, expected 0 before resume")
    if int(pipeline.get("raw_completed_no_paper_candidates") or 0) != int(pipeline.get("not_writable_by_decision_gate") or 0):
        failures.append(
    ...
    if ideas.status == 200 and "Supabase-native" not in str(ideas.body.get("authority", "")):
        failures.append(f"ideas authority does not identify Supabase-native source: {ideas.body.get('authority')!r}")
    ...
    if dispatch_dry.status == 200 and dispatch_dry.body.get("action") != "paused":
        failures.append(f"dispatch dry-run action={dispatch_dry.body.get('action')!r}, expected paused while migration pause is active")
    if dispatch_dry.status == 200 and dispatch_dry.body.get("event_id") is not None:
        failures.append(f"dispatch dry-run recorded an event_id={dispatch_dry.body.get('event_id')}; dry-runs must be side-effect-free")
...
def main() -> int:
    ...
    parser.add_argument("--ssh-host", default=os.environ.get("ENOCH_CONTROL_PLANE_SSH_HOST", ""), help="optional host for systemd timer checks, e.g. root@192.168.1.166")
