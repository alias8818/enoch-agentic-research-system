...
printf '%s\n' "${runtime_paths[@]}" \
  | ssh "$gb10_host" "set -euo pipefail; cd '$gb10_codex_home'; keep=\$(mktemp); trap 'rm -f \"\$keep\"' EXIT; while IFS= read -r path; do
    ...
    tar -cf - -T \"\$keep\"" \
  | ssh "$cpu_host" "set -euo pipefail; stage=\$(mktemp -d); trap 'rm -rf \"\$stage\"' EXIT; tar -C \"\$stage\" -xf -; if find \"\$stage\" -type l -print -quit | grep -q .; then echo 'error: symlink entries are not allowed in sync payload' >&2; exit 1; fi; if find \"\$stage\" -type f -links +1 -print -quit | grep -q .; then echo 'error: hardlink entries are not allowed in sync payload' >&2; exit 1; fi; [[ -f \"\$stage/auth.json\" ]] || { echo 'error: auth.json must be a regular file' >&2; exit 1; }; [[ -f \"\$stage/config.toml\" ]] || { echo 'error: config.toml must be a regular file' >&2; exit 1; }; mkdir -p '$cpu_codex_home' '$cpu_codex_home/.tmp'; root=\$(realpath -m '$cpu_codex_home'); stage_root=\$(realpath -m \"\$stage\"); while IFS= read -r path; do case \"\$path\" in ''|/*|-*|*'..'*) echo \"error: rejecting unsafe sync payload path: \$path\" >&2; exit 1 ;; esac; target=\$(realpath -m '$cpu_codex_home/'\"\$path\"); source=\$(realpath -m \"\$stage/\$path\"); case \"\$target\" in \"\$root\"/*) ;; *) echo \"error: rejecting unsafe sync payload path: \$path\" >&2; exit 1 ;; esac; case \"\$source\" in \"\$stage_root\"/*) ;; *) echo \"error: rejecting unsafe sync payload path: \$path\" >&2; exit 1 ;; esac; rm -rf -- \"\$target\"; if [[ -e \"\$source\" ]]; then parent=\$(dirname \"\$target\"); mkdir -p \"\$parent\"; mv \"\$source\" \"\$target\"; fi; done <<'EOF'
auth.json
config.toml
models_cache.json
version.json
