...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--live", action="store_true", help="query the live control-plane readiness endpoint")
    parser.add_argument("--config", default=os.environ.get("ENOCH_CONFIG", "/etc/enoch-control-plane/config.json"))
    parser.add_argument("--control-url", default="")
    parser.add_argument("--token", default=os.environ.get("ENOCH_CONTROL_TOKEN", ""))
    parser.add_argument("--json", action="store_true", help="print the full readiness payload")
    args = parser.parse_args(argv)
    if not args.live:
        parser.error("--live is required; offline fixture mode is intentionally not implemented")
    ...
    if payload.get("ok"):
        ...
        return 0
    for blocker in payload.get("blockers") or [payload.get("label") or "long-haul readiness failed"]:
