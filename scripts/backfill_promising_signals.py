...
def _load_rows(input_json: Path | None, project_ids: list[str], query: str) -> list[dict[str, Any]]:
    if input_json:
...
def _sanitized_backfilled_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in clean_rows:
        ...
        if "artifact_root" in copied:
            copied["artifact_root"] = exporter._safe_path_text(copied.get("artifact_root"))
        if "artifact_paths" in copied:
            copied["artifact_paths"] = [exporter._safe_path_text(item) for item in exporter._list(copied.get("artifact_paths"))]
        copied = json.loads(json.dumps(copied, default=str))
        sanitized.append(copied)
    sanitized.sort(key=lambda item: (str(item.get("project_id") or ""), str(item.get("run_id") or item.get("current_run_id") or "")))
    return sanitized


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input-json", type=Path, help="JSON array of control-plane rows for offline/deterministic backfill")
    ...
    parser.add_argument("--output-repo", type=Path, help="Optional promising-signals repo to rewrite from the clean backfilled row set")
    ...
    if args.report_markdown:
        ...
        args.report_markdown.write_text(exporter.audit_backfill_markdown(report) + "\n", encoding="utf-8")
    if args.backfilled_rows_json:
        args.backfilled_rows_json.parent.mkdir(parents=True, exist_ok=True)
        args.backfilled_rows_json.write_text(json.dumps(_sanitized_backfilled_rows(rows), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    if args.output_repo:
