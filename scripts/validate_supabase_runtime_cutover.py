...
def _supabase_counts(database_url: str) -> dict[str, Any]:
    with _connect_pg(database_url) as conn:
        with conn.cursor() as cur:
            ...
            dashboard = cur.execute("select * from operator_dashboard_counts").fetchone()
            tables = cur.execute(
                """
...
def _live_counts(control_url: str, token: str) -> dict[str, Any]:
    ...
    return {
        ...
        "raw_completed_no_paper_candidates": int(pipeline.get("raw_completed_no_paper_candidates") or 0),
        "not_writable_by_decision_gate": int(pipeline.get("not_writable_by_decision_gate") or 0),
        "publication_ready": int(pipeline.get("publish_ready") or 0),
        "needs_attention": int(operator_counts.get("needs_attention") or (overview.get("counts") or {}).get("blocked") or 0),
...
def compare(live: dict[str, Any], supabase: dict[str, Any], *, require_safe_pause: bool = True) -> CutoverCheck:
    failures: list[str] = []
    for key in PIPELINE_KEYS:
        if int(live.get(key) or 0) != int(supabase.get(key) or 0):
            failures.append(f"{key} mismatch: live={live.get(key)} supabase={supabase.get(key)}")
...
def main() -> int:
    ...
    if not token:
        failures.append("missing control-plane token; set ENOCH_CONTROL_PLANE_TOKEN or --token-file")
    if not args.database_url.strip():
        failures.append("missing Supabase Postgres URL; set ENOCH_SUPABASE_DATABASE_URL or pass --database-url")
    if failures:
    ...
    try:
        ...
        print(json.dumps({"ok": False, "failures": [f"preflight exception: {type(exc).__name__}: {exc}"]}, indent=2, sort_keys=True))
        return 1
    print(json.dumps({"ok": result.ok, "failures": result.failures, "live": result.live, "supabase": result.supabase}, indent=2, sort_keys=True))
