...
REQUIRED_STANDARD_ARRAY_FIELDS = (
    "expected_artifacts",
    "required_evidence",
                ...
                _tokens(
                    " ".join(
                        [
                            ...
                            _text(row.get("baseline_to_beat")),
                        ]
                    )
                )
            )[:5]
...
def detect_candidate_clusters(
    ...
    *,
    ...
    clusters.sort(
        key=lambda item: (item["candidate_count"], item["mean_similarity"]),
        reverse=True,
    )
    return clusters
...
def _shared_setup_signal(rows: list[dict[str, Any]]) -> bool:
    text = "\n".join(
        " ".join(
            [
                _text(row.get("implementation")),
                _text(row.get("expected_artifacts")),
                _text(row.get("mechanism")),
            ]
        )
        for row in rows
...
def extract_reflection_patterns(
    ...
    for row in rows:
        state = _text(
            row.get("decision_gate_state")
            or row.get("research_outcome")
            or row.get("decision")
            or row.get("status")
        ).lower()
        summary = _text(
            row.get("decision_summary")
            or row.get("summary")
            or row.get("project_decision_summary")
        )
        is_positive = (
            state in SUCCESS_STATES
            or "finalize_positive" in state
            or "paper_positive" in state
        )
        is_useful = (
            is_positive
            or "useful" in state
            or (state in USEFUL_STATES and "useful" in summary.lower())
        )
...
def build_synthesis_prompt(
    cluster: dict[str, Any], reflection_patterns: list[dict[str, Any]]
) -> str:
    compact_candidates = [
        {
...
def enrich_synthesized_candidate(
    candidate: dict[str, Any],
    ...
    *,
    ...
    row.setdefault(
        "source_urls", [cluster.get("source_url")] if cluster.get("source_url") else []
    )
    ...
    row["raw_candidate_json"] = {
        **(
            ...
            if isinstance(row.get("raw_candidate_json"), dict)
            else {}
        ),
        "synthesized_from": source_ids,
        "reflection_source_ids": [
            _text(pattern.get("project_id"))
            for pattern in reflection_patterns
            if _text(pattern.get("project_id"))
        ],
    ...
    *,
    ...
    def call(prompt: str) -> dict[str, Any]:
        ...
        if not checked.get("ok"):
            ...
            raise RuntimeError(
                "; ".join(_text(item) for item in failures if _text(item))
            )
...
def _provider_from_args(args: argparse.Namespace) -> Callable[[str], dict[str, Any]]:
    if args.provider_response_json:
        payload = json.loads(
            Path(args.provider_response_json).read_text(encoding="utf-8")
        )
    ...
    def call(prompt: str) -> dict[str, Any]:
        ...
        return research_provider_generate.extract_json_object(
            research_provider_generate._extract_chat_content(payload)
        )
    ...
    def budget_check() -> dict[str, Any]:
        ...
        payload = research_provider_budget.fetch_json(
            ...
            api_key=""
            if args.budget_no_auth
            else (args.provider_api_key or os.environ.get("SYNTHETIC_API_KEY", "")),
            timeout=args.timeout,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--history-json", type=Path, help="optional prior successes/useful signals JSON"
    )
    parser.add_argument("--output", type=Path, help="write synthesis report JSON")
    parser.add_argument(
        "--emit-sql",
        type=Path,
        help="write SQL for synthesized candidate and branch deferral",
    )
    ...
    parser.add_argument(
        ...
        default="",
        help="test/offline provider response JSON",
    )
    parser.add_argument(
        "--provider-base-url",
        default=research_provider_generate.DEFAULT_OPENAI_BASE_URL,
    )
    parser.add_argument(
        "--provider-model", default=research_provider_generate.DEFAULT_MODEL
    )
    ...
    parser.add_argument(
        "--budget-base-url", default=research_provider_budget.SYNTHETIC_BASE_URL
    )
    ...
    if args.emit_sql:
        args.emit_sql.write_text(
            emit_synthesis_sql(
                report,
                requested_by=args.requested_by,
                queue_synthesized=args.queue_synthesized,
            ),
            encoding="utf-8",
        )
