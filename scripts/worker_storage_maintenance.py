...
def _allocated_file_bytes(path: Path, seen: set[tuple[int, int]]) -> int:
    try:
...
def unique_allocated_bytes(candidates: Iterable[CleanupCandidate]) -> int:
    ...
    for candidate in candidates:
        for child in candidate.path.rglob("*"):
            total += _allocated_file_bytes(child, seen)
    return total
...
def _cleanup_candidate_for(
    ...
    return CleanupCandidate(
        ...
        protected=protected,
        protect_reason=f"project {project} is protected" if protected else "",
    )


def _should_skip_child_directory(child: Path, root: Path) -> bool:
    if child.is_symlink():
        return True
...
def main(argv: list[str] | None = None) -> int:
    ...
    should_apply = _maybe_skip_apply_for_free_space(root=root, args=args, report=report)
    if should_apply:
