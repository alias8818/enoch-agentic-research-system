...
# Centralized filename for the duplicated DRAFT_TEX literal
...
# Centralized filename for the duplicated MANIFEST_JSON literal
# (addresses current top remaining S1192 in artifact name lists and path assignments).
...
def _check_parity(name: str, sqlite_val: Any, pg_val: Any, failures: list[str]) -> None:
    if sqlite_val != pg_val:
        failures.append(f"{name} mismatch")


def _start_postgres_container(container: str) -> str:
    run(
        [
            "docker",
            "run",
    ...
    return pg_url(container)


def _validate_readonly_store_parity(
    sqlite_store: ControlPlaneStore,
    pg_store: SupabaseReadOnlyControlPlaneStore,
    failures: list[str],
    ...
    _check_parity(
        ...
        sqlite_store.latest_dashboard_observation(source="worker_preflight").payload,
        pg_store.latest_dashboard_observation(source="worker_preflight").payload,
        failures,
    )  # type: ignore[union-attr]
    return queue_keys, paper_keys


def _verify_readonly_rejects_pause(
    pg_store: SupabaseReadOnlyControlPlaneStore, failures: list[str]
) -> None:
    try:
        pg_store.pause(
...
def _verify_append_event_idempotency(
...
def _run_write_smoke(
    ...
    _assert_ok(
        ...
        and write_store.latest_dashboard_observation(source="worker_preflight")
        is not None,
...
def _verify_final_queue_counts(
    write_store: SupabaseControlPlaneStore, failures: list[str]
) -> None:
...
def _run_notion_intake_smoke(
    ...
    return notion_project_id


def _run_paper_review_smoke(
    ...
    artifact_root.mkdir()
    for rel_path in (DRAFT_MD, DRAFT_TEX, EVIDENCE_JSON, CLAIMS_JSON, MANIFEST_JSON):
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            url = _start_postgres_container(container)
            postgres_fixture(url)
            pg_store = SupabaseReadOnlyControlPlaneStore(url)
            write_store = SupabaseControlPlaneStore(url)

            failures: list[str] = []
            queue_keys, paper_keys = _validate_readonly_store_parity(
                sqlite_store, pg_store, failures
            )
            ...
            report = _build_validation_report(
                failures, write_store, pg_store, queue_keys, paper_keys
            )
