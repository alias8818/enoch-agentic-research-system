...
it('shows research signal quality below the command-center secondary fold', async () => {
  ...
  const quality = within(secondaryFold as HTMLElement).getByLabelText('Research signal quality')
  ...
  expect(within(quality).queryByText('Follow-up readiness')).not.toBeInTheDocument()
  expect(within(quality).queryByText('ready follow-ups 1 / 2 recommended')).not.toBeInTheDocument()
  expect(within(quality).queryByText('underspecified 1')).not.toBeInTheDocument()
  expect(within(quality).queryByText('missing stop 1')).not.toBeInTheDocument()
  expect(within(quality).queryByText('Mixed follow-up must improve accuracy by 5 points.')).not.toBeInTheDocument()
  expect(within(quality).queryByText('Prioritized follow-up')).not.toBeInTheDocument()
  expect(within(quality).queryByText('1 recommended follow-up is underspecified; fill missing readiness fields before queueing it')).not.toBeInTheDocument()
...
it('surfaces secondary top actions while maintenance blocks the primary action', async () => {
  ...
  const secondaryFold = screen.getByText('Show secondary details').closest('details')
  expect(secondaryFold).not.toBeNull()
  const topActions = within(secondaryFold as HTMLElement).getByLabelText('Top actions')
  expect(topActions).not.toBeVisible()
  fireEvent.click(screen.getByText('Show secondary details'))
  ...
  expect(within(topActions).getByRole('link', { name: 'Open candidate generation' })).toHaveAttribute('href', '/control/dashboard-v2#research')
  expect(within(topActions).queryByRole('link', { name: 'Queue follow-up' })).not.toBeInTheDocument()
