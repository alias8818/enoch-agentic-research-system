...
export function QueuePage({ route }: { route: Extract<DashboardRoute, { page: 'queue' }> }) {
  ...
  return (
    <PageShell title="Queue" subtitle="Bounded queue rows from /control/api/v1/queue. No frontend lifecycle inference.">
      <DataTable rows={query.data?.rows || []} columns={['project_id', 'status', 'lane', 'machine_target', 'title', 'updated_at']} empty="No queue rows match this filter." />
    </PageShell>
  )
}
...
export function PapersPage({ route }: { route: Extract<DashboardRoute, { page: 'papers' }> }) {
  ...
  return (
    <PageShell title="Papers" subtitle="Paper pipeline rows from /control/api/v1/papers.">
      <DataTable rows={query.data?.rows || []} columns={['paper_id', 'project_id', 'status', 'title', 'artifact_dir', 'updated_at']} empty="No paper rows match this filter." />
    </PageShell>
  )
}
...
export function EventsPage() {
  ...
  return (
    <PageShell title="Events" subtitle="Recent formatted control-plane events from /control/api/v1/events.">
      <DataTable rows={query.data?.rows || []} columns={['id', 'entity_type', 'entity_id', 'event_type', 'created_at', 'summary']} empty="No recent events returned." />
    </PageShell>
  )
}
