...
it('loads observability health and memory from backed V1 endpoints', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'Owl output empty' })).toBeInTheDocument()
  ...
  expect(screen.getByRole('heading', { name: 'Research agents pool needs tuning' })).toBeInTheDocument()
  ...
  expect(screen.queryByText('Latest redacted model preview')).not.toBeInTheDocument()
