...
describe('CommandHero state strip', () => {
  it('shows only active and queued counts; paper counts belong in PaperMiniStrip', () => {
    ...
    expect(screen.getByRole('link', { name: 'Write9' })).toHaveTextContent('9')
    expect(screen.getByRole('button', { name: 'Dry-run finalize' })).toHaveTextContent('4')
    expect(screen.getByRole('link', { name: 'Publish1' })).toHaveTextContent('1')
  })
})
