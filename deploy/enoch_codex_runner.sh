...
PY_CALLBACK_PAYLOAD
  ...
  if printf '%s' "$CALLBACK_TOKEN" | python3 -m enoch_control_plane.callback_outbox deliver --state-dir "$WORKER_STATE_DIR" --run-id "$RUN_ID" --url "$CALLBACK_URL" --token-stdin --timeout "$CALLBACK_TIMEOUT"; then
    true
  else
